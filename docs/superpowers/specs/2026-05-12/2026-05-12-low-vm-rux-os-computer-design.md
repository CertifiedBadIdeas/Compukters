...
- Rux programs that can eventually behave like kernel, shell, drivers, and user programs;
- a reusable VM core for computers and future simpler devices.

If the computer is meant to feel like a small real machine, then processes should not be a hidden Rust runtime feature forever. A Rux OS gives the project one coherent mental model: Rust emulates the machine; Rux software defines what runs on it.
...
Those are Rux kernel responsibilities.
...
The first Rux OS slice should use one boot CPU. Multiple CPU contexts can remain an experimental capability, but the Rux kernel MVP should not depend on SMP or parallel CPUs.
...
2. Rust loads one Rux kernel image.
3. Rust creates one boot CPU context.
4. The boot CPU starts at the image entry function.
5. Rux kernel initializes a small OS state block in RAM.
6. Rux kernel writes machine status through the existing control MMIO registers:
   - `RESET`
   - `BOOTING`
   - `READY`
   - `PANIC`
...
The first Rux kernel should prove only these concepts:
...
The Rux OS should eventually expose syscalls to user programs. The first slice should define the ABI shape but avoid implementing broad services.
...
- user programs ask the Rux kernel for services;
- Rux kernel talks to Rust devices through MMIO or machine traps;
...
- no display, filesystem, terminal, or storage device in the first Rux OS slice.
...
- current Rux compatibility path.
...
3. Add a minimal Rux/low-image kernel fixture that writes `BOOTING`, initializes an OS state block, then writes `READY`.
...
- A Rux kernel fixture can mark the machine ready through MMIO.
- A Rux kernel fixture can mark the machine panicked through MMIO.
...
- A Rux OS can easily become too large before any product value appears. Keep each slice bootable and testable.
- The current Rux compiler may not yet express enough low-level constructs. Kernel fixtures can start as low-image builders or generated fixtures until Rux catches up.
