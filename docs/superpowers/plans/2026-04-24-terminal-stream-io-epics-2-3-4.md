...
> **Status (2026-04-24):** Epic 2 delivered (tasks 2.1–2.9). Epic 3 delivered (tasks 3.1, 3.2, 3.3, 3.5; task 3.4 is effectively a no-op while the snapshot path lives). Epic 4 partially delivered: task 4.3 (DECTCEM cursor blink via VT) is in; tasks 4.1 (remove hardcoded 77×27 + rework `ComputerProfile`), 4.2 (delete `ComputerTerminalClientMessage`, `ComputerMenuWithoutInventory`, duplicate sinks), the original task 2.5 (remove `screenBuffer` from `ComputerTerminalApi`) and 2.10 (retire the legacy client message) are **still pending** — they form one invasive cutover because deleting the server-side `ScreenBuffer` breaks the Workbench snapshot path, which then forces Workbench (task 3.4) to migrate to `ClientTerminalBuffer` too. Do that cutover together in a dedicated branch.
