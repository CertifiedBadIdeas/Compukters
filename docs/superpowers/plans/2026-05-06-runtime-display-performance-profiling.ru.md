# План реализации профилирования производительности runtime display

> **Для agentic workers:** REQUIRED SUB-SKILL: Используйте superpowers:subagent-driven-development (рекомендуется) или superpowers:executing-plans для реализации этого плана task-by-task. Steps используют checkbox (`- [ ]`) syntax для tracking.

**Goal:** Добавить profiling-only instrumentation для runtime display/terminal workloads, чтобы будущие оптимизации опирались на evidence.

**Architecture:** Добавляем optional no-op-by-default metrics collector на display boundary, подключаем его в `DisplayRegistry` и `BackgroundDeviceVm`, затем добавляем reusable bundled ROM terminal workload test, который записывает display operation counts, frame/tile/payload counts и broad tick diagnostics. Этот план не оптимизирует rendering и не добавляет новые display APIs.

**Tech Stack:** Kotlin, Gradle, kotlin.test/JUnit 5, CKL ROM resources, существующий `DisplayRegistry`/`DisplayFrameDelta` runtime display stack.

---

## File structure

- Create `modules/core/src/main/kotlin/ru/lazyhat/compukterkraft/core/device/vm/display/DisplayProfiling.kt` — data classes и no-op/recording metrics collector.
- Modify `modules/core/src/main/kotlin/ru/lazyhat/compukterkraft/core/device/vm/display/DisplayRegistry.kt` — запись display operation metrics и frame metrics.
- Modify `modules/core/src/main/kotlin/ru/lazyhat/compukterkraft/core/device/vm/BackgroundDeviceVm.kt` — optional `DisplayMetricsCollector` и передача его в `DisplayRegistry`.
- Create `modules/core/src/test/kotlin/ru/lazyhat/compukterkraft/core/device/vm/display/DisplayProfilingTest.kt` — unit tests для collector behavior и registry wiring.
- Create `modules/v1_21_1/v1_21_1-neoforge/src/test/kotlin/ru/lazyhat/compukterkraft/impl/computer/vm/RuntimeDisplayProfilingTest.kt` — integration profiling scenario для bundled firmware/ROM terminal.
