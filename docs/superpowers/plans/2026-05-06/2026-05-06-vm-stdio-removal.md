# VM Stdio/Terminal Removal Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove CKL `terminal`/`stdout` runtime APIs and VM-side stdio plumbing, while preserving child process errors through tagged VM-local stderr channels that programs render themselves.

**Architecture:** Visible runtime output remains display-only and program-owned. `process::run`/`spawn` keep their current signatures; the runtime decodes a tagged `stdio-v1 <stdin> <stdout> <stderr> <argument>` descriptor from the argument and writes child loader/compiler/runtime errors to that stderr IPC channel when present. Terminal/stdout builtins, bridge handlers, VM APIs, and VM-owned screen-buffer plumbing are removed after bundled firmware/ROM scripts stop using them.
...
Implementation worktree has been created and baseline-verified:
...
Run all commands below from `/home/lazyhat/IdeaProjects/Compukter-Kraft/.worktrees/vm-stdio-removal`.
...
- Modify: `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/runtime/DeviceVmModels.kt`
  - remove `readScreenSnapshot()` and `forceScreenSnapshot()` from `DeviceVmHandle` if call sites can be removed.
- Modify: `modules/core/src/main/kotlin/ru/lazyhat/compukterkraft/core/device/runtime/RuntimeDevice.kt`
  - remove `RuntimeDeviceScreen` if Workbench snapshot path becomes no-op/removable.
- Modify: `modules/core/src/main/kotlin/ru/lazyhat/compukterkraft/core/device/runtime/RuntimeDeviceImpl.kt`
    - remove `syncScreen()` and `lastScreenSnapshot`.
...
## Task 1: Add tagged stdio descriptor parser
...
Expected: FAIL at compile time because `StdioDescriptor` does not exist.
...
## Task 3: Route process manager errors to tagged stderr
...
- [ ] **Step 1: Add failing VM stderr tests**

In core `BackgroundDeviceVmTest.kt`, add a test using static firmware that opens IPC channels, runs a missing child with a tagged descriptor, reads stderr, and writes it to the server log through `system::log`:
...
Add a second test for compile errors:
...
```kotlin
        ...
        suspend fun reportError(message: String) {
            ...
            if (stderr != null && stderr >= 0) {
                ctx.ipcWrite(stderr, message + "\n")
            }
        }
```

If `VmContext` does not expose `ipcWrite`, add a narrow method to `VmContext` implemented by `BackgroundDeviceVm` that delegates to `ipcRegistry.write(channel, text)`. Name it exactly:

```kotlin
suspend fun writeIpc(channel: Int, text: String)
```
...
and replace compile/runtime error prints the same way.
...
## Task 4: Remove terminal/stdout builtins from compiler registry
...
In `LanguageIdeTest.kt`, add or update a test so module completions no longer contain terminal/stdout:
...
Adapt constructor names to the existing `LanguageIdeTest` helper if it already wraps `LanguageIde` differently.
...
In `LanguageBuiltins.kt`, delete the entire `BuiltinModule(name = "terminal", ...)` and `BuiltinModule(name = "stdout", ...)` entries.
...
For tests that only need a side-effect-free valid program, replace old snippets with one of these:
...
## Task 5: Remove runtime bridge and DeviceRuntime terminal/stdout APIs
...
## Task 6: Remove core VM terminal/stdout implementations
...
## Task 7: Remove VM screen snapshot ownership path
...
## Task 8: Update firmware and ROM to render without terminal/stdout
