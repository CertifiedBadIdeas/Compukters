...
> **Status (2026-04-24):** Epic 2 delivered (tasks 2.1–2.10). Epic 3 delivered (tasks 3.1, 3.2, 3.3, 3.5; task 3.4 still pending — Workbench remains on the snapshot path). Epic 4 mostly delivered: 4.2 (delete `ComputerTerminalClientMessage` + `MenuSide.Client.screenSnapshot` + `ComputerContainerData.terminalSnapshot`) and 4.3 (DECTCEM cursor blink via VT) are in; 4.1 (remove hardcoded 77×27 + rework `ComputerProfile`) and task 3.4 (migrate Workbench to byte-stream) remain and depend on the Workbench cutover — separate branch.

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Prerequisite:** Epic 1 (`docs/superpowers/plans/2026-04-24/2026-04-24-terminal-stream-io-epic-1.md`) is complete. VM emits a VT-100 byte stream via `ComputerStdioApi`; server-side `VmStdioApi` currently feeds it back into a ScreenBuffer. That bridge is the thing Epic 2 replaces.
