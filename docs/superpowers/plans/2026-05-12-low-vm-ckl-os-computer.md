# Low VM CKL OS Computer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the first bootable CKL OS experiment slice where `ComputerMachine` owns shared RAM, MMIO, and boot CPU contexts while CKL/low-image fixtures initialize OS state through guest memory.

**Architecture:** Extract a reusable `LowCpuContext` from `LowImageCpu<'_>` so CPU execution state no longer borrows machine memory for its whole lifetime. Then make `ComputerMachine` own CPU contexts and run them against its shared `MachineBus`, proving the CKL OS direction with ready and panic boot fixtures.

**Tech Stack:** Rust `ckl-vm`, low VM image model, `MachineMemory`, `MachineBus`, MMIO control registers, Rust unit/integration tests, Cargo.
...
Add this test near the existing external-memory tests in `native/ckl-vm/tests/low_image_runner.rs`:
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml cpu_context_runs_against_shared_memory_without_owning_it
```
...
In `native/ckl-vm/src/low_image_runner.rs`, replace the current `LowImageCpu<'memory>` shape with a context-owned core plus compatibility wrapper:
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml --test low_image_runner
```
...
Add this test in `native/ckl-vm/src/computer_machine.rs`:
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml computer_machine_owns_boot_cpu_context
```
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml computer_machine
```
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml boot_kernel_initializes_os_state_and_marks_machine_ready
```
...
```bash
git add native/ckl-vm/src/computer_machine.rs
git commit -m "Add CKL OS ready boot fixture"
```
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml boot_kernel_can_panic_through_control_mmio
```
...
```bash
git add native/ckl-vm/src/computer_machine.rs
git commit -m "Add CKL OS panic boot fixture"
```
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml computer_machine_rejects
```
