# Low VM Rux OS Static User Process Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Prove that a Rux/low-image kernel can use a guest RAM process descriptor to launch a statically linked user function and record its exit status in the guest process table.
...
**Tech Stack:** Rust `rux-vm`, low VM `Instruction::CallStatic`, `ComputerMachine`, guest RAM process table fixtures, Cargo tests.
...
```bash
cargo test --manifest-path native/rux-vm/Cargo.toml kernel_launches_static_user_process_and_records_exit_code
```
...
git commit -m "Add Rux OS static user process fixture"
```
