...
**Architecture:** Rust code is grouped under `rust/`: host-side crates live in `rust/host`, and guest-side crates live in `rust/guest`. `rust/guest/k16-abi` contains no-std machine constants and typed MMIO helpers. `rust/guest/k16-rt` contains no-std compiler/runtime helpers such as `memcpy`, `memmove`, `memset`, and `memcmp`; BIOS, bootloader, and kernel crates will link it instead of duplicating those helpers.

**Tech Stack:** Rust 2021, Cargo workspace, `#![no_std]`, Rust unit tests through `cargo test --manifest-path rust/guest/Cargo.toml`.
...
**Files:**
- Create: `rust/guest/Cargo.toml`
- Create: `rust/guest/k16-abi/Cargo.toml`
- Create: `rust/guest/k16-abi/src/lib.rs`
- Create: `rust/guest/k16-rt/Cargo.toml`
- Create: `rust/guest/k16-rt/src/lib.rs`

- [x] **Step 1: Write failing runtime helper tests**

Create unit tests in `rust/guest/k16-rt/src/lib.rs` that call `k16_memcpy`, `k16_memmove`, `k16_memset`, and `k16_memcmp` before those helpers exist.

- [x] **Step 2: Run tests to verify RED**

Run: `cargo test --manifest-path rust/guest/Cargo.toml`
