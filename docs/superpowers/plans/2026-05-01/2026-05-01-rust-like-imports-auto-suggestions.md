# Rust-like Imports and Auto Suggestions Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add explicit selective imports for built-in namespaces and `.ck` files, remove flat file imports, and provide auto-import completions with right-side source labels.

**Architecture:** Model imports as `source + mode`, resolve selective imports into ordinary visible bindings, and keep namespace imports for `::` access. Add completion metadata and text edits so the workbench can insert the chosen name and update an import group atomically. Add a lightweight workspace source index for importable user-file symbols without changing deterministic compiler semantics.
...
- Modify `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/api/ImportDeclaration.kt`: replace the flat import data class with source/mode/item model.
...
- Modify `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/frontend/FrontendPipelines.kt`: traverse selective file imports while building the project parse cache.
- Modify `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/frontend/SourceLoader.kt`: add source-index abstractions and map-backed implementation.
- Modify `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/runtime/DeviceWorkspaceSourceLoader.kt`: implement workspace source indexing for device workspaces.
- Modify `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/runtime/DeviceIdeHost.kt`: add completion source-label and text-edit metadata.
- Modify `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/frontend/LanguageIde.kt`: produce importable completions and import edits.
- Modify `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/frontend/SourceTextSupport.kt`: support import-group contexts and import insertion helpers.
- Modify `modules/compiler/src/main/kotlin/ru/lazyhat/compukterkraft/lang/frontend/IdePresentationSupport.kt`: fill source labels for normal symbol completions when available.
- Modify `modules/core/src/main/kotlin/ru/lazyhat/compukterkraft/core/workbench/WorkbenchStore.kt`: apply completion edits atomically and keep cursor placement.
- Modify `modules/core/src/main/kotlin/ru/lazyhat/compukterkraft/core/workbench/screen/WorkbenchUiBuilder.kt`: render completion label on the left and source label on the right.
- Modify `modules/core/src/main/kotlin/ru/lazyhat/compukterkraft/core/device/vm/WorkspaceDeviceIdeHost.kt` and `modules/v1_21_1/v1_21_1-common/src/main/kotlin/ru/lazyhat/compukterkraft/common/infrastructure/workbench/WorkbenchGateways.kt`: pass workspace source index into IDE completion paths.
...
- Modify `docs/LANGUAGE.md`: replace flat import docs with selective import docs.
