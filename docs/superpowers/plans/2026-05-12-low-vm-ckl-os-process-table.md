# Low VM CKL OS Process Table Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Prove that process-table and scheduler state can live in guest RAM and be owned by the CKL/low-image kernel rather than Rust.
...
**Tech Stack:** Rust `ckl-vm`, low VM `Instruction` fixtures, `ComputerMachine`, `MachineMemory`, Cargo tests.

---

## Memory Layout

Use the existing test-only OS state constants in `native/ckl-vm/src/computer_machine.rs`.
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml boot_kernel_initializes_guest_process_table
```
...
git commit -m "Add CKL OS process table boot fixture"
```
...
```bash
cargo test --manifest-path native/ckl-vm/Cargo.toml scheduler_fixture_rotates_running_process_state
```
...
```bash
git add native/ckl-vm/src/computer_machine.rs
git commit -m "Add CKL OS scheduler state fixture"
```
