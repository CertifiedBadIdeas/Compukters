...
| `kraft_std::process::run_with_args(path, args)` | `k16_rt::run_argv_syscall(request, len)` | Runs a dynamic foreground child program with `1..=4` bounded argv byte strings. |
