...
```text
.toolchain/build/llvm/k16-min/bin
```
...
- `k16 disasm --target program --start 0x8000 --count ...` sees the linked
  startup path and LLVM-produced `main` without decoding any following
