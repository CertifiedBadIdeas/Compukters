...
The [Guest API reference]({{ '/guest-api/' | relative_url }}) is generated by Dokka with separate
[Guest Kotlin]({{ '/guest-api/guest/' | relative_url }}) and
[Create addon]({{ '/guest-api/create/' | relative_url }}) modules. Create signatures link to Guest Kotlin types;
