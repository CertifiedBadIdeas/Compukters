...
| `./gradlew-sandbox-dev-parallel-summary verifyLocalFull` | Complete current checkout: all subproject checks, the standalone Create addon against the Maven-published local SDK, registered conformance, Rust/FFI/JNI and Runtime-version consistency, integrations, real GameTests, and both production artifacts for the locally configured native platform | Clean-tag state or Linux-and-Windows universal release readiness |
