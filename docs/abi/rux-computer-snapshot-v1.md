...
`debug`, `display0`, serial input, and the `storage0` controller.
...
`storage0` media contents are not stored in `RUXSNAP`; they remain part of the
...
`storage0` controller registers from the snapshot against an explicitly
