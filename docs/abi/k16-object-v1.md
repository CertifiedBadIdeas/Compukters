...
`--import <library>:<symbol>` records or `--dylib <library.kso>` inputs,
...
```text
k16-ld <rustc linker args> --k16-target <bios|boot|kernel|program|program-dynamic|shared-object> [--import <library>:<symbol>] [--dylib <library.kso>] [--map <output.map>] -o <output>
```
...
object when they require the wider CSR, interrupt, and trap helper surface.
