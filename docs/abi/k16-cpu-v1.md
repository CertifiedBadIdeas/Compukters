...
| `RUN` | `9` | `syscall3(RUN, request, len, format)` | Synchronously loads a dynamic K16E user program through VFS from `/bin/*.kx` on writable storage0 or `/sdk/bin/*.kx` on optional read-only storage1, blocks the current foreground process while the child runs, and returns either a non-negative child exit status or a negative K16 error. Child status `0` is success; non-zero child statuses are successful launches whose child process reported failure. Negative values are launch/runtime errors such as missing executable, invalid executable, no memory, busy child slot, or translated user fault. `trap_arg2 = 1` means `trap_arg0/trap_arg1` point to a bounded argv request block beginning with `RUN_ARGV_MAGIC`; `argc = 0` is valid for zero-argument launches. Other `trap_arg2` formats are invalid. |
| `OPEN` | `10` | `syscall3(OPEN, path, len, flags)` | Opens an absolute KFS regular-file path through VFS. `flags = OPEN_READ_ONLY` may read storage0 or mounted `/sdk`; the descriptor retains its owning volume. Write/create/truncate/append flags operate on storage0 and return `ERROR_READ_ONLY` for `/sdk`. Success returns a regular file fd starting at `3`. |
...
| `READ_DIR` | `14` | `syscall3(READ_DIR, request, len, 0)` | Reads a KFS directory listing through VFS from storage0 or mounted `/sdk` into a caller-provided buffer. The request is `u32 magic`, `u32 path_len`, `u32 out_ptr`, `u32 out_len`, followed by path bytes. The output buffer receives packed little-endian records: `u32 file_type`, `u32 name_len`, `name_len` raw name bytes, `u32 size_bytes`. |
| `STAT` | `15` | `syscall3(STAT, path, len, metadata)` | Reads KFS path metadata through VFS from storage0 or mounted `/sdk` into a 16-byte response buffer. Word 0 is `FILE_TYPE_REGULAR` or `FILE_TYPE_DIRECTORY`; word 1 is `size_bytes`; words 2 and 3 are reserved zero. |
...
| `UNLINK` | `18` | `syscall3(UNLINK, path, len, 0)` | Removes an absolute KFS regular-file path from storage0. Directories are rejected, open files return `ERROR_BUSY`, and `/sdk` returns `ERROR_READ_ONLY`. |
| `MKDIR` | `19` | `syscall3(MKDIR, path, len, 0)` | Creates an absolute KFS directory on storage0. Parent directories must exist; `/sdk` returns `ERROR_READ_ONLY`. |
| `RMDIR` | `20` | `syscall3(RMDIR, path, len, 0)` | Removes an empty KFS directory from storage0. Regular files are rejected, non-empty directories return `ERROR_NOT_EMPTY`, and `/sdk` returns `ERROR_READ_ONLY`. |
| `RENAME` | `21` | `syscall3(RENAME, request, len, 0)` | Renames an absolute KFS regular file on storage0 without changing its inode or contents. The request is `u32 magic`, `u32 old_path_len`, `u32 new_path_len`, followed by old path bytes and new path bytes. Directories are rejected, existing destinations return `ERROR_INVALID`, open sources return `ERROR_BUSY`, and either path under `/sdk` returns `ERROR_READ_ONLY`. |
...
| `ERROR_READ_ONLY` | `0xffff_ffe2` | n/a | Negative K16 error value corresponding to read-only-filesystem semantics for `/sdk`. |
...
All `/sdk` mutations are rejected before filesystem block I/O. The storage1
