...
| `rust/host/k16-vm/src/computer/devices.rs` | Host implementations of K16 MMIO devices: control, debug serial, serial input, display0, framebuffer0, storage0, and BIOS flash. |
...
- `FramebufferDevice` turns guest RAM blits into `DisplayFrameDelta` values.
