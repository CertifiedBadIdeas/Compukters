...
import ru.lazyhat.compuktercraft.network.client.ChatHelpers.coloured
...
interface TableFormatter {
    ...
    fun display(table: TableBuilder) {
        ...
        if (table.additional > 0) {
            writeLine(
                id,
                Component.translatable("commands.compuktercraft.generic.additional_rows", table.additional).withStyle(ChatFormatting.AQUA),
            )
        }
    }
