...
fn kfs_storage_helper_is_kfs_owned() {
    ...
    let workspace_manifest = root.join("guest/kraftos/Cargo.toml");
    let kernel_manifest = root.join("guest/kraftos/kernel/Cargo.toml");
    let kernel_error = root.join("guest/kraftos/kernel/src/kfs/error.rs");
    let kernel_types = root.join("guest/kraftos/kernel/src/kfs/types.rs");
    let kernel_device = root.join("guest/kraftos/kernel/src/kfs/device.rs");
    let kernel_fs = root.join("guest/kraftos/kernel/src/fs.rs");

    let workspace_manifest =
        fs::read_to_string(&workspace_manifest).expect("K16 guest workspace manifest exists");
    let kernel_manifest = fs::read_to_string(&kernel_manifest).expect("K16 kernel manifest exists");
    let kernel_error =
        fs::read_to_string(&kernel_error).expect("kernel-owned storage error source exists");
    let kernel_types =
        fs::read_to_string(&kernel_types).expect("kernel-owned KFS public types source exists");
    let kernel_device =
        fs::read_to_string(&kernel_device).expect("kernel-owned storage device source exists");
    let kernel_fs = fs::read_to_string(&kernel_fs).expect("kernel-owned FS facade source exists");
