...
pub fn assemble_k16_f32(source: &str, base: u32, entry: &str) -> Result<AssembledK16F32, String> {
    if !base.is_multiple_of(4) {
        return Err(format!(
            "K16-F32 base {base:#010x} is not four-byte aligned"
        ));
    }
    ...
    for record in &parsed {
        ...
        } = record
        {
            ...
            address = address
                ...
                .ok_or_else(|| format!("line {line}: K16-F32 image address overflow"))?;
            words.extend(encoded);
        }
    }
    let stop_offset = instruction_count
        .checked_mul(4)
        .ok_or("K16-F32 image size overflow")?;
    words.push(encoding::halt());
    let image = words.into_iter().flat_map(u32::to_le_bytes).collect();
    ...
    for (index, original) in source.lines().enumerate() {
        ...
        if line.starts_with('.') {
            ...
            if !matches!(
                ...
                return Err(format!(
                    "line {line_number}: unknown K16-F32 assembler directive {directive:?}"
                ));
            }
            continue;
        }
    ...
    for record in parsed {
        match record {
            ...
            ParsedLine::Instruction { line, mnemonic, .. } => {
                let emitted = if mnemonic == "const32" { 2 } else { 1 };
...
fn encode_instruction(
    ...
    let encoded = match mnemonic {
        "const32" => {
            ...
            let register = parse_register(&operands[0]).map_err(&fail)?;
            let value = parse_u32_bits(&operands[1]).map_err(&fail)?;
            return Ok(encoding::materialize(register, value).to_vec());
        }
        "addi" => {
            ...
            encoding::addi(
                ...
                parse_i16(&operands[2], "immediate").map_err(&fail)?,
            )
        }
        ...
        "br" | "brz" | "brnz" | "call32" => {
            let (register, target_operand) = match mnemonic {
                "brz" | "brnz" => {
                    require_operands(line, mnemonic, operands, 2)?;
                    (
            ...
            let offset = relative_offset(target_operand, address, labels, 20).map_err(&fail)?;
            match mnemonic {
        ...
        "brltu" | "bruge" => {
            ...
            let offset = relative_offset(&operands[2], address, labels, 16).map_err(&fail)?;
            let offset = i16::try_from(offset).map_err(|_| {
                fail(format!(
                    "relative target {:?} does not fit i16",
                    operands[2]
                ))
            })?;
            if mnemonic == "brltu" {
        ...
        _ => return Err(fail(format!("unknown K16-F32 opcode {mnemonic:?}"))),
    };
    Ok(vec![encoded])
}
...
fn parse_register(text: &str) -> Result<u8, String> {
    ...
    if number < 16 {
        ...
        Err(format!("register {text:?} is outside r0..r15"))
    }
}

fn parse_i16(text: &str, kind: &str) -> Result<i16, String> {
    let value = parse_integer(text).map_err(|error| format!("invalid {kind}: {error}"))?;
    i16::try_from(value).map_err(|_| format!("{kind} {text:?} does not fit i16"))
}
...
fn parse_memory(text: &str) -> Result<(u8, i16), String> {
    let inner = text
        .strip_prefix('[')
    ...
    match pieces.as_slice() {
        ...
        [base, "+", offset] => Ok((parse_register(base)?, parse_i16(offset, "memory offset")?)),
        [base, "-", offset] => {
            let value =
                parse_integer(offset).map_err(|error| format!("invalid memory offset: {error}"))?;
            let value = value
                .checked_neg()
            ...
            Ok((
                parse_register(base)?,
                i16::try_from(value)
                    .map_err(|_| format!("memory offset -{offset} does not fit i16"))?,
            ))
        }
