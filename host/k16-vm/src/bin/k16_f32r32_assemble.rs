...
fn main() {
    if std::env::args().nth(1).as_deref() == Some("--help") {
        println!(
            "usage: k16_f32r32_assemble --base ADDRESS --entry SYMBOL --input ASM --output IMAGE --manifest MANIFEST"
        );
        return;
    }
    if let Err(error) = run() {
        eprintln!("k16_f32r32_assemble: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let assembled = assemble_k16_f32r32(&source, arguments.base, &arguments.entry)?;
