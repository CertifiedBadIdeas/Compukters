...
impl K16F32R32Cpu {
    ...
    pub fn set_register(&mut self, register: usize, value: u32) -> Result<(), String> {
        let destination = self
            .registers
            .get_mut(register)
            .ok_or_else(|| format!("K16-F32R32 register index {register} is outside 0..32"))?;
        *destination = value;
        Ok(())
    }
    ...
    pub fn step(&mut self, bus: &mut dyn MemoryBus) -> Result<Option<K16F32R32Stop>, String> {
        if !self.pc.is_multiple_of(4) {
            return Err(format!(
                "misaligned K16-F32R32 instruction PC {:#010x}",
                self.pc
            ));
        }
    ...
    fn execute(
        ...
        match instruction {
            ...
            DecodedInstruction::Halt => return Ok(Some(K16F32R32Stop::Halt)),
            DecodedInstruction::Yield => return Ok(Some(K16F32R32Stop::Yield)),
            DecodedInstruction::Lui { dst, immediate } => self.registers[dst] = immediate << 13,
            ...
            DecodedInstruction::Call { offset } => {
                self.registers[31] = next_pc;
                self.pc = relative(offset);
            }
            DecodedInstruction::Ret => self.pc = self.registers[31],
        }
        Ok(None)
    }
}
...
fn require_alignment(address: u32, alignment: u32, pc: u32) -> Result<(), String> {
    if address.is_multiple_of(alignment) {
        ...
        Err(format!(
            "misaligned K16-F32R32 access {address:#010x} at PC {pc:#010x}"
        ))
    }
}
