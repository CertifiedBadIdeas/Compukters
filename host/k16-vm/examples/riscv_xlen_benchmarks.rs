...
fn main() -> Result<(), String> {
    ...
    for artifact in artifacts {
        ...
        let observation = run_compiled_c(&artifact, iterations, max_steps)?;
    ...
    for sample_index in 0..sample_count {
        for index in rotated_order(candidates.len(), sample_index) {
            let measurement = &mut candidates[index];
    ...
    println!();
    println!("RISC-V XLEN comparison");
    print!("{comparison}");
    Ok(())
}
...
fn rotated_order(count: usize, round: usize) -> Vec<usize> {
    (0..count).map(|index| (index + round) % count).collect()
}
