...
use k16_vm::compiled_c::artifact::{
    load_compiled_c_artifact, validate_artifact_pair, CompiledCArtifact,
};
use k16_vm::compiled_c::report::{
    format_decision, format_timing_sample, populate_vs_native, timing_report_header,
    CompiledCTiming,
};
...
fn main() -> Result<(), String> {
    ...
    if arguments.next().is_some() {
        return Err(
            "usage: isa_gate2_benchmarks <artifact-root> <iterations> <warm-samples>".to_string(),
        );
    }
    ...
    let decision = format_decision(&timings)?;

    println!("ISA Gate 2 compiled-C benchmark");
...
fn load_artifacts(root: &Path) -> Result<Vec<CompiledCArtifact>, String> {
    ...
    for workload in IsaBenchmarkWorkload::all().iter().copied().take(6) {
        ...
        let k16 = load_compiled_c_artifact(&directory.join("k16-f32.manifest"))?;
        let rv = load_compiled_c_artifact(&directory.join("rv32im.manifest"))?;
        validate_artifact_pair(&k16, &rv)?;
        artifacts.push(k16);
        artifacts.push(rv);
    }
    Ok(artifacts)
