...
impl K16RootFs {
    ...
    pub unsafe fn read_directory_into<S: DirectoryListingSink>(
        ...
        let selected_metadata =
            unsafe { crate::storage::select_inode_metadata_for_cache(directory_inode_id)? };
    ...
    unsafe fn resolve_path(
        ...
        let mut inode_id = unsafe { crate::storage::root_inode_id() };
        ...
        while index < path.len() {
            ...
            match self.cache.lookup_directory(inode_id, component) {
                ...
                None => {
                    unsafe { crate::storage::select_inode_metadata_for_cache(inode_id)? };
                    let child_inode_id =
                        unsafe { crate::storage::selected_directory_entry_inode(component)? };
                    let child_metadata =
                        unsafe { crate::storage::select_inode_metadata_for_cache(child_inode_id)? };
