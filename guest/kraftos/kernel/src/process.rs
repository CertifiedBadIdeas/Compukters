...
const SDK_BIN_PREFIX: &[u8] = b"/sdk/bin/";
const KX_SUFFIX: &[u8] = b".kx";
const KSO_SUFFIX: &[u8] = b".kso";
const KFS_MAX_NAME_BYTES: usize = 56;
pub const MAX_RUN_PATH_BYTES: usize = SDK_BIN_PREFIX.len() + KFS_MAX_NAME_BYTES;
...
pub unsafe fn begin_translated_init_from_storage0(
    ...
    if UserProgramPath::parse(path)?.volume() != crate::vfs::VolumeId::Root {
        return Err(ProcessLoadError::InvalidPath);
    }
    ...
    let mapped_child_plan =
        unsafe { load_dynamic_user_program_mapped(path, arena, &mut allocator)? };
...
pub unsafe fn begin_loaded_child_from_path(path: &[u8]) -> Result<ChildLaunch, u32> {
    ...
    #[cfg(test)]
    {
        ...
        let child_plan = unsafe { load_dynamic_user_program(path, arena) }
            .map_err(run_status_from_load_error)?;
        table
...
pub unsafe fn begin_loaded_child_from_argv_request(request: &[u8]) -> Result<ChildLaunch, u32> {
    ...
    #[cfg(test)]
    {
        ...
        let arena = table
            .child_arena_for_init_frame(init_frame)
            .map_err(run_status_from_load_error)?;
        let child_plan = unsafe { load_dynamic_user_program(request.path, arena) }
            .map_err(run_status_from_load_error)?;
...
unsafe fn load_runtime_child(path: &[u8], args: &[&[u8]]) -> Result<ProcessId, u32> {
    ...
    if translated_child {
        let allocator = unsafe { (*RUNTIME_PAGE_ALLOCATOR.get()).as_mut() }
            .ok_or_else(|| run_status_from_load_error(ProcessLoadError::Storage))?;
        let mapped_child_plan = unsafe { load_dynamic_user_program_mapped(path, arena, allocator) }
            .map_err(run_status_from_load_error)?;
        let child_plan = mapped_child_plan.virtual_plan();
    ...
    let child_plan =
        unsafe { load_dynamic_user_program(path, arena) }.map_err(run_status_from_load_error)?;
    let argv =
...
impl<'a> UserProgramPath<'a> {
    pub fn parse(path: &'a [u8]) -> Result<Self, ProcessLoadError> {
        let (volume, name) = if let Some(name) = path.strip_prefix(BIN_PREFIX) {
            (crate::vfs::VolumeId::Root, name)
        } else if let Some(name) = path.strip_prefix(SDK_BIN_PREFIX) {
            (crate::vfs::VolumeId::Sdk, name)
        } else {
            return Err(ProcessLoadError::InvalidPath);
        };
        if name.is_empty()
    ...
    pub const fn volume(&self) -> crate::vfs::VolumeId {
        self.volume
    }
}
...
fn program_profile_file(path: UserProgramPath<'_>) -> FileReadProfileFile {
...
fn library_profile_file(name: &[u8]) -> FileReadProfileFile {
...
impl ResidentSharedLibrary {
    ...
    fn matches(&self, name: &[u8], file: FileMetadata) -> bool {
...
impl DynamicImportLibrary {
    const fn empty() -> Self {
        Self {
            ...
            name_len: 0,
            file: empty_file_metadata(),
            profile_file: FileReadProfileFile::Generic,
            image: SharedLibraryImage::empty(),
            relative_base: 0,
            cache_slot: 0,
        }
    ...
    #[cfg(test)]
    fn profile_file(&self) -> FileReadProfileFile {
        self.profile_file
    }
...
const fn empty_file_metadata() -> FileMetadata {
    FileMetadata {
        inode_id: 0,
        size_bytes: 0,
        extent_count: 0,
...
pub unsafe fn load_dynamic_user_program(
    path: &[u8],
    arena: UserArena,
) -> Result<DynamicUserLoadPlan, ProcessLoadError> {
    crate::os_stats::record_program_load();
    let parsed_path = UserProgramPath::parse(path)?;
    let profile_file = program_profile_file(parsed_path);
    let file =
        unsafe { crate::fs::open_file_cached_path(path).map_err(|_| ProcessLoadError::Storage)? };
    if file.volume != parsed_path.volume() {
        return Err(ProcessLoadError::Storage);
    }
    let version = unsafe { selected_k16e_version(file, profile_file)? };
    if version == 5 || version == 6 {
        unsafe {
            load_selected_dynamic_user_program_with_imports(file, arena, version, profile_file)
        }
    } else {
        unsafe { load_selected_dynamic_user_program(file, arena, profile_file) }
    }
}

pub unsafe fn load_dynamic_user_program_mapped(
    path: &[u8],
    arena: UserArena,
    allocator: &mut crate::page_alloc::PageFrameAllocator,
) -> Result<MappedDynamicUserLoadPlan, ProcessLoadError> {
    ...
    if version == 5 || version == 6 {
        ...
        unsafe { load_selected_dynamic_user_program_mapped(file, arena, allocator, profile_file) }
    }
}

unsafe fn copy_vfs_file_range_to_ram_profiled(
    file: crate::fs::VfsFile,
    file_offset: u32,
    dst_addr: u32,
    len: u32,
    ...
    unsafe {
        crate::vfs::with_volume(file.volume, |volume| {
            crate::kfs::file_io::copy_file_range_to_ram_profiled(
                volume,
                file.metadata,
                file_offset,
                dst_addr,
...
pub(crate) unsafe fn load_selected_dynamic_user_program(
    file: crate::fs::VfsFile,
    arena: UserArena,
    profile_file: FileReadProfileFile,
) -> Result<DynamicUserLoadPlan, ProcessLoadError> {
    unsafe {
        copy_vfs_file_range_to_ram_profiled(
            file,
            0,
    ...
    validate_dynamic_header_bytes(header, file.metadata.size_bytes)?;
    ...
    unsafe {
        copy_vfs_file_range_to_ram_profiled(
            ...
            plan.payload_dst,
            plan.payload_len,
            FileReadProfileKind::Program(profile_file),
        )?;
    }
...
unsafe fn load_selected_dynamic_user_program_with_imports(
    ...
    let state = unsafe { read_selected_dynamic_import_state(file, version, profile_file)? };
...
pub(crate) unsafe fn load_selected_dynamic_user_program_mapped(
    file: crate::fs::VfsFile,
    arena: UserArena,
    allocator: &mut crate::page_alloc::PageFrameAllocator,
    profile_file: FileReadProfileFile,
...
unsafe fn read_selected_dynamic_import_image(
    ...
    if version == 6 {
        ...
        return Ok(DynamicUserImportImage {
            ...
            needed_section_offset: if has_imports {
                header_u32(header, 100)
            } else {
                0
            },
            needed_section_size: if has_imports {
                header_u32(header, 104)
            } else {
                0
            },
            needed_library_count: if has_imports {
                header_u32(header, 108)
            } else {
                0
            },
            import_section_offset: if has_imports {
                header_u32(header, 120)
            } else {
                0
            },
            import_section_size: if has_imports {
                header_u32(header, 124)
            } else {
                0
            },
            import_relocation_count: if has_imports {
                header_u32(header, 128)
            } else {
                0
            },
            writable_offset,
...
unsafe fn read_selected_dynamic_import_state(
    ...
    let main_file = file;
    let image = unsafe { read_selected_dynamic_import_image(file, version, profile_file)? };
    ...
    while index < image.needed_library_count {
        let mut name = [0u8; KFS_MAX_NAME_BYTES];
...
unsafe fn apply_dynamic_import_relocations(
    ...
    while index < image.import_relocation_count {
        let mut symbol = [0u8; KFS_MAX_NAME_BYTES];
        let relocation =
...
unsafe fn open_shared_library(name: &[u8]) -> Result<FileMetadata, ProcessLoadError> {
...
unsafe fn find_resident_shared_library(name: &[u8], file: FileMetadata) -> Option<u32> {
    let mut index = 0;
...
unsafe fn read_resident_shared_library(
    ...
    let file_handle = crate::fs::VfsFile {
        volume: crate::vfs::VolumeId::Root,
        metadata: file,
    };
    let image = unsafe { read_shared_library_image(file_handle, profile_file)? };
    ...
    unsafe {
        copy_vfs_file_range_to_ram_profiled(
            ...
            image.file_size,
            FileReadProfileKind::Library(profile_file),
        )?;
        copy_vfs_file_range_to_ram_profiled(
            file_handle,
            image.relocation_table_offset,
...
fn read_needed_library_name<'a>(
    ...
    section: &[u8],
    index: u32,
    out: &'a mut [u8; KFS_MAX_NAME_BYTES],
...
fn read_import_relocation_record<'a>(
    ...
    section: &[u8],
    index: u32,
    symbol_out: &'a mut [u8; KFS_MAX_NAME_BYTES],
...
unsafe fn read_shared_library_image(
    file: crate::fs::VfsFile,
    profile_file: FileReadProfileFile,
    ...
    let image = shared_library_image_from_header(header, file.metadata.size_bytes)?;
    crate::os_stats::record_dynamic_import_bytes(header_size);
    Ok(image)
}
...
mod tests {
    ...
    fn assert_mmu_map(writes: &[(u32, u32)], virtual_start: u32, page_count: u32, flags: u32) {
        let index = writes
            .iter()
    ...
    #[test]
    fn user_program_path_accepts_sdk_bin_kx_path() {
        let path = UserProgramPath::parse(b"/sdk/bin/tcc.kx").expect("SDK path parses");

        assert_eq!(
            path.components(),
            &[b"bin".as_slice(), b"tcc.kx".as_slice()]
        );
        assert_eq!(path.volume(), crate::vfs::VolumeId::Sdk);
    }
    ...
    fn dynamic_import_library_keeps_opened_file_metadata() {
        ...
        let library = DynamicImportLibrary {
            ...
            name_len: 0,
            file: metadata,
            profile_file: FileReadProfileFile::OtherLibrary,
            image: SharedLibraryImage::empty(),
            relative_base: 0,
            cache_slot: 0,
        };
        ...
        assert_eq!(library.profile_file(), FileReadProfileFile::OtherLibrary);
    }
    ...
    fn dynamic_library_cache_entry_matches_name_and_file_metadata() {
        ...
        let changed_metadata = FileMetadata {
            size_bytes: 8192,
            ..metadata
        };
    ...
    fn dynamic_user_load_plan_uses_kernel_selected_arena_base_and_stack() {
        ...
        let plan = plan_dynamic_user_load(arena, DynamicUserImage::new(2, 8, 12))
    ...
    fn dynamic_user_load_plan_rejects_program_that_would_reach_stack() {
        ...
        assert_eq!(
            plan_dynamic_user_load(arena, DynamicUserImage::new(0, 8, 16),),
    ...
    fn dynamic_user_load_plan_rejects_invalid_dynamic_image_metadata() {
        ...
        assert_eq!(
            plan_dynamic_user_load(arena, DynamicUserImage::new(13, 8, 12),),
            Err(ProcessLoadError::InvalidImage)
        );
    }
    ...
    fn init_loaded_image_defines_child_arena_after_init_image() {
        ...
        let plan = plan_dynamic_user_load(arena, DynamicUserImage::new(0, 8, 16))
    ...
    fn child_arena_rejects_program_that_would_overlap_live_init_stack() {
        ...
        assert_eq!(
            plan_dynamic_user_load(arena, DynamicUserImage::new(0, 0x400, 0x400),),
            Err(ProcessLoadError::ProgramTooLarge)
        );
    }
    ...
    fn child_arena_uses_live_init_stack_as_child_stack_top() {
        ...
        let plan = plan_dynamic_user_load(arena, DynamicUserImage::new(0, 0x400, 0x400))
