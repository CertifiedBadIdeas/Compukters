...
`native/k16-vm` and `native/rux-compiler`. Documentation is in `docs/`, active machine ABI references are in `docs/abi`,
...
- `cargo test` from `native/k16-vm` or `native/rux-compiler` runs Rust crate tests.
