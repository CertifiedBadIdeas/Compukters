...
class VmFileSystemApi(
    ...
    override suspend fun exists(path: String): Boolean =
        ctx.awaitHostCall { HostCall.FileExists(it, ctx.resolvePath(path)) }

    override suspend fun isDirectory(path: String): Boolean =
        ctx.awaitHostCall { HostCall.FileIsDirectory(it, ctx.resolvePath(path)) }

    override suspend fun readText(path: String): String? =
        ctx.awaitHostCall { HostCall.FileReadText(it, ctx.resolvePath(path)) }
    ...
    override suspend fun makeDirectory(path: String): Boolean =
        ctx.awaitHostCall { HostCall.FileMakeDirectory(it, ctx.resolvePath(path)) }

    override suspend fun remove(path: String): Boolean =
        ctx.awaitHostCall { HostCall.FileRemove(it, ctx.resolvePath(path)) }
