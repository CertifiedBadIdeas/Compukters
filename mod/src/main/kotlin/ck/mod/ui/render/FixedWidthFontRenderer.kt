...
object FixedWidthFontRenderer {
    ...
    private fun drawTerminalBackground(
        ...
        if (topMarginSize > 0) {
            ...
            drawQuad(emitter, x - leftMarginSize, y - topMarginSize, 0f,
                snapshot.width * FONT_WIDTH.toFloat() + leftMarginSize + rightMarginSize, topMarginSize, colour, FULL_BRIGHT_LIGHTMAP)
        }
        // Bottom margin
        if (bottomMarginSize > 0) {
            val colour = Palette.DEFAULT.getRenderColours(15 - snapshot.bgAt(0, snapshot.height - 1))
            drawQuad(emitter, x - leftMarginSize, y + snapshot.height * FONT_HEIGHT, 0f,
                snapshot.width * FONT_WIDTH.toFloat() + leftMarginSize + rightMarginSize, bottomMarginSize, colour, FULL_BRIGHT_LIGHTMAP)
        }
        ...
        for (row in 0 until snapshot.height) {
            ...
            if (rightMarginSize > 0) {
                ...
                drawQuad(emitter, x + snapshot.width * FONT_WIDTH, rowY, 0f, rightMarginSize, FONT_HEIGHT.toFloat(), colour, FULL_BRIGHT_LIGHTMAP)
            }
            ...
            for (col in 1 until snapshot.width) {
                ...
                if (bg != blockBg) {
                    ...
                    drawQuad(emitter, x + blockStart * FONT_WIDTH, rowY, 0f,
                        ((col - blockStart) * FONT_WIDTH).toFloat(), FONT_HEIGHT.toFloat(), colour, FULL_BRIGHT_LIGHTMAP)
                    blockStart = col
                    blockBg = bg
                }
            }
            val colour = Palette.DEFAULT.getRenderColours(15 - blockBg)
            drawQuad(emitter, x + blockStart * FONT_WIDTH, rowY, 0f,
                ((snapshot.width - blockStart) * FONT_WIDTH).toFloat(), FONT_HEIGHT.toFloat(), colour, FULL_BRIGHT_LIGHTMAP)
        }
    }
