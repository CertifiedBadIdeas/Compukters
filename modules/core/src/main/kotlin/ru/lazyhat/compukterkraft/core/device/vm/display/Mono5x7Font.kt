...
internal object Mono5x7Font {
    const val WIDTH: Int = 5
    const val HEIGHT: Int = 7
    const val CELL_ADVANCE: Int = 6

    private const val UNKNOWN: Long = 0b11111100011000110001100011000111111L

    private val glyphs: LongArray =
        LongArray(128) { UNKNOWN }.also { glyphs ->
            ...
            for (code in 65 until 91) glyphs[code + 32] = glyphs[code]
        }
    ...
    fun rowBits(
        ch: Char,
        row: Int,
    ): Int = ((packedBits(ch) shr ((HEIGHT - 1 - row) * WIDTH)) and 0b11111L).toInt()
}
