...
/**
 * The `ru.lazyhat.compukters.core.device.runtime` package is the home of
 * [ru.lazyhat.compukters.core.device.runtime.RuntimeDevice] and its
...
class RuntimePackagePurityTest {
    private val forbiddenImportPatterns =
        listOf(
            ...
            Regex("""^\s*import\s+ru\.lazyhat\.compukters\.common\."""),
            Regex("""^\s*import\s+ru\.lazyhat\.compukters\.impl\."""),
        )
    ...
    fun runtimePackageContainsNoPlatformOrCarrierImports() {
        val runtimeRoot =
            findRepoRoot()
                .resolve("modules/core/src/main/kotlin/ru/lazyhat/compukters/core/device/runtime")
        assertTrue(
