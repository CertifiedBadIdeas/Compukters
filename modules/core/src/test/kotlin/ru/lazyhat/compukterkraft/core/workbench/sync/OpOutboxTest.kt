...
class OpOutboxTest {
    ...
    private fun insertOp(
        clock: Int,
        text: String = "x",
    ): Op.Insert = Op.Insert(author = site, clock = clock, leftId = null, text = text)

    @Test
    fun debouncesEnqueuesIntoSingleSend() =
        runTest {
    ...
    @Test
    fun maxBatchTriggersImmediateFlush() =
        runTest {
            val sent = mutableListOf<List<Op>>()
    ...
    @Test
    fun flushNowEmitsAndClearsQueue() =
        runTest {
            val sent = mutableListOf<List<Op>>()
    ...
    @Test
    fun ackForLastSentClockTransitionsToIdle() =
        runTest {
    ...
    @Test
    fun staleAfterTimeoutWithoutAck() =
        runTest {
    ...
    @Test
    fun newEnqueueAfterFlushSchedulesAnotherDebounce() =
        runTest {
            val sent = mutableListOf<List<Op>>()
            val outbox = OpOutbox(this, send = { sent.add(it) }, debounceMs = 50, maxBatch = 64)
    ...
    @Test
    fun deleteOpUsesItsOwnClockForAckTracking() =
        runTest {
            val outbox = OpOutbox(this, send = {}, debounceMs = 0, maxBatch = 64)
