...
class WorkbenchTerminalClientMessage : NetworkMessage<ClientNetworkContext> {
    private val containerId: Int
    private val terminalState: TerminalState?

    constructor(menu: AbstractContainerMenu, snapshot: ScreenBufferSnapshot?) {
        containerId = menu.containerId
        terminalState = snapshot?.let(::TerminalState)
    }

    constructor(buf: FriendlyByteBuf) {
        containerId = buf.readVarInt()
        terminalState = if (buf.readBoolean()) TerminalState(buf) else null
    }

    override fun write(buf: FriendlyByteBuf) {
        buf.writeVarInt(containerId)
        buf.writeBoolean(terminalState != null)
        terminalState?.write(buf)
    }

    override fun handle(context: ClientNetworkContext) {
        context.handleWorkbenchTerminal(containerId, terminalState?.toSnapshot())
    }

    override fun type(): MessageType<WorkbenchTerminalClientMessage> = NetworkMessages.WORKBENCH_TERMINAL
}