...
data class SourceSnapshotId(
    val hash: Hash256,
)

object SourceSnapshotIdentity {
    fun of(snapshot: ProjectSnapshot): SourceSnapshotId {
        val digest = MessageDigest.getInstance("SHA-256")
        digest.update(DOMAIN)
        digest.updateLittleEndian(snapshot.sources.size)
        ...
        return SourceSnapshotId(Hash256.of(digest.digest()))
    }
    ...
    private val DOMAIN = "Compukters source snapshot v1\u0000".encodeToByteArray()
}
