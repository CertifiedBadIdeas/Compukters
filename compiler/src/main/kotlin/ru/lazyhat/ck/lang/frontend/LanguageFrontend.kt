...
import ru.lazyhat.ck.lang.api.BinaryExpression
import ru.lazyhat.ck.lang.api.BinaryOperator
import ru.lazyhat.ck.lang.api.BlockStatement
import ru.lazyhat.ck.lang.api.BoolLiteralValue
...
import ru.lazyhat.ck.lang.api.BytecodeFunction
import ru.lazyhat.ck.lang.api.BytecodeLocal
import ru.lazyhat.ck.lang.api.BytecodeModule
import ru.lazyhat.ck.lang.api.BytecodeRecord
import ru.lazyhat.ck.lang.api.CallExpression
import ru.lazyhat.ck.lang.api.Expression
import ru.lazyhat.ck.lang.api.ExpressionStatement
import ru.lazyhat.ck.lang.api.FunctionDeclaration
import ru.lazyhat.ck.lang.api.GroupExpression
import ru.lazyhat.ck.lang.api.IfStatement
import ru.lazyhat.ck.lang.api.ImportDeclaration
import ru.lazyhat.ck.lang.api.Instruction
import ru.lazyhat.ck.lang.api.IntLiteralValue
import ru.lazyhat.ck.lang.api.LiteralExpression
import ru.lazyhat.ck.lang.api.LongLiteralValue
import ru.lazyhat.ck.lang.api.MemberAccessExpression
import ru.lazyhat.ck.lang.api.NameExpression
import ru.lazyhat.ck.lang.api.NullLiteralValue
import ru.lazyhat.ck.lang.api.ParameterDeclaration
import ru.lazyhat.ck.lang.api.Program
import ru.lazyhat.ck.lang.api.RecordConstructionExpression
import ru.lazyhat.ck.lang.api.RecordFieldDeclaration
import ru.lazyhat.ck.lang.api.RecordFieldDefinition
import ru.lazyhat.ck.lang.api.RecordFieldInitializer
import ru.lazyhat.ck.lang.api.ReturnStatement
import ru.lazyhat.ck.lang.api.SourceLocation
import ru.lazyhat.ck.lang.api.SourceRange
import ru.lazyhat.ck.lang.api.Statement
import ru.lazyhat.ck.lang.api.StringLiteralValue
import ru.lazyhat.ck.lang.api.StructDeclaration
import ru.lazyhat.ck.lang.api.Token
import ru.lazyhat.ck.lang.api.TokenKind
import ru.lazyhat.ck.lang.api.TopLevelDeclaration
import ru.lazyhat.ck.lang.api.TypeSyntax
import ru.lazyhat.ck.lang.api.UnaryExpression
import ru.lazyhat.ck.lang.api.UnaryOperator
import ru.lazyhat.ck.lang.api.VariableDeclarationStatement
import ru.lazyhat.ck.lang.api.WhileStatement
import java.util.IdentityHashMap

enum class FrontendSeverity {
    INFO,
    WARNING,
    ERROR,
}

data class FrontendDiagnostic(
    val message: String,
    val range: SourceRange? = null,
    val severity: FrontendSeverity = FrontendSeverity.ERROR,
)
...
data class SymbolInfo(
    ...
    val ownerFunctionRange: SourceRange? = null,
)

data class ReferenceInfo(
    val name: String,
    val range: SourceRange,
    val target: SymbolInfo,
            ...
            modules =
                listOf(
                    BuiltinModule(
                        ...
                        functions =
                            listOf(
                                BuiltinFunction("write", listOf("String"), "Unit", "Writes text without advancing the cursor."),
                                BuiltinFunction("printLine", listOf("String"), "Unit", "Writes text and moves to the next line."),
                                BuiltinFunction("clear", emptyList(), "Unit", "Clears the terminal contents."),
                                BuiltinFunction("setCursor", listOf("Int", "Int"), "Unit", "Moves the terminal cursor."),
                            ),
                    ),
                    BuiltinModule(
                        name = "filesystem",
                        ...
                        functions =
                            listOf(
                                BuiltinFunction("exists", listOf("String"), "Bool", "Returns true when a workspace path exists."),
                                BuiltinFunction("readText", listOf("String"), "String", "Reads a text file from the workspace."),
                                BuiltinFunction("writeText", listOf("String", "String"), "Unit", "Writes a text file in the workspace."),
                            ),
                    ),
                    BuiltinModule(
                        name = "system",
                        ...
                        functions =
                            listOf(
                                ...
                                BuiltinFunction("label", emptyList(), "String", "Returns the computer label or an empty string."),
                                BuiltinFunction("profileName", emptyList(), "String", "Returns the current computer profile name."),
                    ...
                    BuiltinModule(
                        ...
                        functions =
                            listOf(
                                BuiltinFunction("pull", emptyList(), "Event", "Waits until the next event is available."),
                                BuiltinFunction("pull", listOf("String"), "Event", "Waits for an event with the requested name."),
                            ),
                    ),
                ),
            globals =
                listOf(
                    BuiltinFunction("yield", emptyList(), "Unit", "Voluntarily yields execution back to the scheduler."),
                    BuiltinFunction("sleep", listOf("Long"), "Unit", "Suspends the current program for the specified number of ticks."),
                ),
            builtinTypes =
                listOf(
...
private class SemanticAnalyzer(
    ...
    private val typeNames: MutableMap<String, TypeRef> =
        registry.builtinTypes.associate { it.name to TypeRef(it.name) }.toMutableMap()
    ...
    private fun registerImports(imports: List<ImportDeclaration>) {
        imports.forEach { declaration ->
            ...
            if (module == null) {
                diagnostics += FrontendDiagnostic("Unknown module `${declaration.moduleName}`.", declaration.range)
                return@forEach
            }
            val symbol =
                SymbolInfo(
            ...
            symbols += symbol
            importedModules[declaration.moduleName] = ModuleBinding(symbol, module)
        }
    }
    ...
    private fun registerTopLevel(declarations: List<TopLevelDeclaration>) {
        declarations.filterIsInstance<StructDeclaration>().forEach { declaration ->
            ...
            val fields =
                declaration.fields.associate { field ->
                    field.name to (resolveType(field.type, field.range) ?: TypeRef("Unit"))
                }
            val symbol =
                SymbolInfo(
            ...
            val binding = RecordBinding(symbol, declaration, fields)
            symbols += symbol
        ...
        declarations.filterIsInstance<FunctionDeclaration>().forEach { declaration ->
            if (userFunctionsByName.containsKey(declaration.name)) {
                diagnostics += FrontendDiagnostic("Function `${declaration.name}` is already defined.", declaration.range)
                return@forEach
            }
            val parameterTypes = declaration.parameters.map { resolveType(it.type, it.range) ?: TypeRef("Unit") }
            val returnType = declaration.returnType?.let { resolveType(it, it.range) } ?: TypeRef("Unit")
            val symbol =
                SymbolInfo(
                    name = declaration.name,
            ...
            symbols += symbol
            val binding = FunctionBinding(symbol, declaration, parameterTypes, returnType)
    ...
    private fun analyzeFunction(declaration: FunctionDeclaration) {
        ...
        declaration.parameters.forEachIndexed { index, parameter ->
            ...
            symbols += symbol
            scope.define(parameter.name, VariableBinding(symbol, type))
        }
    ...
    private fun analyzeLiteral(expression: LiteralExpression): TypeRef =
        when (expression.value) {
            is BoolLiteralValue -> TypeRef("Bool")
            is IntLiteralValue -> TypeRef("Int")
            is LongLiteralValue -> TypeRef("Long")
            is StringLiteralValue -> TypeRef("String")
            NullLiteralValue -> TypeRef("Unit", nullable = true)
        }
    ...
    private fun analyzeName(
        ...
        if (local != null) {
            ...
            references += ReferenceInfo(expression.name, expression.range, local.symbol, local.type.displayName)
            return local.type
        }
        importedModules[expression.name]?.let { module ->
            localBindings[expression] = module
            references += ReferenceInfo(expression.name, expression.range, module.symbol)
            return TypeRef(module.symbol.name)
        }
        val record = userRecordsByName[expression.name]
        if (record != null) {
            localBindings[expression] = record
            references += ReferenceInfo(expression.name, expression.range, record.symbol, record.symbol.name)
            return TypeRef(record.symbol.name)
        }
        diagnostics += FrontendDiagnostic("Unknown symbol `${expression.name}`.", expression.range)
        return TypeRef("Unit")
    }
    ...
    private fun analyzeMember(
        ...
        references += ReferenceInfo(expression.memberName, expression.range, symbol, fieldType.displayName)
        return MemberBinding(symbol, receiverType, fieldType) to fieldType
    }
    ...
    private fun analyzeCall(
                ...
                is NameExpression -> {
                    ...
                    if (global != null) {
                        FunctionBinding(
                            ...
                            returnType = TypeRef(global.returnType),
                        )
                    } else {
        ...
        references += ReferenceInfo(binding.symbol.name, expression.callee.range, binding.symbol, binding.returnType.displayName)
    ...
    private fun analyzeRecordConstruction(
        ...
        if (record == null) {
            diagnostics += FrontendDiagnostic("Unknown struct `${expression.typeName}`.", expression.range)
            return TypeRef("Unit")
        }
        expression.fields.forEach { field ->
            val expected = record.fields[field.name]
            if (expected == null) {
                diagnostics += FrontendDiagnostic("Struct `${record.symbol.name}` has no field `${field.name}`.", field.range)
            } else {
        ...
        return when (expression.operator) {
            UnaryOperator.NEGATE -> {
                if (operandType.name !in setOf("Int", "Long")) {
                    diagnostics += FrontendDiagnostic("Unary minus expects Int or Long.", expression.range)
                }
                operandType
            }

            UnaryOperator.NOT -> {
                expectAssignable(operandType, TypeRef("Bool"), expression.range, "Logical not expects Bool.")
                TypeRef("Bool")
            }
        }
    }
    ...
    private fun expectAssignable(
        ...
        if (!isAssignable(actual, expected)) {
            diagnostics += FrontendDiagnostic("$message Expected ${expected.displayName}, got ${actual.displayName}.", range)
        }
    }
    ...
    private inner class FunctionCompiler(
        ...
        private fun compileExpression(expression: Expression) {
            when (expression) {
                ...
                is NameExpression -> {
                    val binding = semantic.localBindings[expression]
                    when (binding) {
                        is VariableBinding -> instructions += Instruction.LoadLocal(resolveLocalSlot(binding.symbol.name))
                        else -> instructions += Instruction.PushUnit
                    }
                }
