...
pub extern "C" fn main(argc: u32, argv: *const process::Arg) -> ! {
    match rmdir_args(argc, argv) {
        Ok(()) => process::exit(0),
        Err(()) => process::exit(1),
    }
}

fn rmdir_args(argc: u32, argv: *const process::Arg) -> Result<(), ()> {
...
fn remove_path(path: &str) -> Result<(), ()> {
    ...
    match fs::remove_dir(path) {
        ...
        Err(fs::Error::Syscall(status)) => {
            write_rmdir_error(
                stdout,
                status::syscall_status_name_or(status, b"RMDIR"),
                path,
            )?;
            Err(())
        }
...
fn write_rmdir_error(stdout: io::Fd, name: &[u8], path: &str) -> Result<(), ()> {
