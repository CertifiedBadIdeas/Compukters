...
pub mod host_test {
    pub use k16_abi::syscall::{
        ...
        ERROR_NO_ENTRY, ERROR_NO_MEMORY, EXIT, FD_STDERR, FD_STDIN, FD_STDOUT, OPEN, READ,
        READ_DIR, RUN, RUN_FORMAT_ARGV, SBRK, SLEEP_TICKS, STAT, STATUS_OK, WRITE, YIELD,
    };
...
pub use trap::{
    ...
    enable_interrupts, exit_syscall, install_trap_vector, interrupt_pending, iret_once,
    iret_with_r0, open_syscall, read_dir_syscall, read_syscall, restore_trap_frame,
    run_argv_syscall, run_syscall, save_trap_frame, sbrk_syscall, set_interrupt_mask,
    sleep_ticks_syscall, stat_syscall, syscall0, syscall1, syscall3, syscall_arg0, syscall_arg1,
    syscall_arg2, syscall_once, trap_cause, trap_pc, trap_value, write_syscall, yield_syscall,
    TrapFrame,
};
