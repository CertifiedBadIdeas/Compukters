...
pub const COMPUTER_SNAPSHOT_V1_K16_CPU_RECORD_SIZE: usize = 208;
...
fn encode_cpu_record(
    ...
    match record {
        ComputerCpuSnapshotRecord::K16 { cpu, max_steps } => {
            ...
            write_u32(bytes, cpu.trap_kernel_stack_pointer);
        }
    }
    Ok(())
}
...
fn decode_cpu_record(bytes: &[u8], index: u32) -> Result<ComputerCpuSnapshotRecord, String> {
    ...
    let trap_kernel_stack_pointer = read_u32(bytes, 204)?;
