...
pub const K16FS_MAGIC: &[u8; 5] = b"K16FS";
pub const K16FS_VERSION: u8 = 1;
pub const K16FS_BLOCK_SIZE: usize = 512;
pub const K16FS_INODE_SIZE: u32 = 64;
pub const K16FS_DEFAULT_INODE_COUNT: u32 = 64;
...
impl InodeState {
    fn from_byte(value: u8) -> Result<Self, String> {
        match value {
            ...
            _ => Err(format!("unsupported K16FS inode state {value}")),
        }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct K16FsExtent {
    pub start_block: u32,
    pub block_count: u32,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct K16FsInode {
    pub state: InodeState,
    pub flags: u32,
    pub size_bytes: u64,
    pub extents: Vec<K16FsExtent>,
}
...
pub fn format_empty_filesystem(total_blocks: u32) -> Result<Vec<u8>, String> {
    if total_blocks < 16 {
        return Err("K16FS requires at least 16 blocks".to_string());
    }
    let bitmap_block_count = bitmap_blocks_for(total_blocks)?;
    let inode_table_block_count = inode_table_blocks_for(K16FS_DEFAULT_INODE_COUNT)?;
    ...
    let root_dir_block = inode_table_start_block
        ...
        .ok_or_else(|| "K16FS metadata range overflows".to_string())?;
    if root_dir_block >= total_blocks {
        return Err("K16FS metadata does not fit in filesystem".to_string());
    }
    let superblock = K16FsSuperblock {
        block_size: K16FS_BLOCK_SIZE as u32,
        total_blocks,
        bitmap_start_block,
        bitmap_block_count,
...
pub fn read_file(image: &[u8], path: &str) -> Result<Vec<u8>, String> {
    ...
    if parsed.is_empty() {
        return Err("K16FS file path must not be root".to_string());
    }
    let superblock = decode_superblock(image)?;
    validate_filesystem(image)?;
    ...
    if inode.state != InodeState::File {
        return Err(format!("K16FS path `{path}` is not a file"));
    }
...
pub fn decode_superblock(image: &[u8]) -> Result<K16FsSuperblock, String> {
    if image.len() < K16FS_BLOCK_SIZE {
        return Err("K16FS superblock is truncated".to_string());
    }
    if &image[0..5] != K16FS_MAGIC {
        return Err("invalid K16FS magic".to_string());
    }
    let version = image[5];
    if version != K16FS_VERSION {
        return Err(format!("unsupported K16FS version {version}"));
    }
    if image[6] != 0 || image[7] != 0 {
        return Err("K16FS reserved superblock bytes must be zero".to_string());
    }
    Ok(K16FsSuperblock {
...
pub fn decode_inode(
    ...
    inode_id: u32,
) -> Result<K16FsInode, String> {
    let offset = inode_offset(superblock, inode_id)?;
    let bytes = image
        .get(offset..offset + K16FS_INODE_SIZE as usize)
        .ok_or_else(|| format!("K16FS inode {inode_id} is outside filesystem"))?;
    ...
    if extent_count > K16FS_MAX_INLINE_EXTENTS {
        return Err(format!(
            "K16FS inode {inode_id} has unsupported extent count"
        ));
    }
...
pub fn validate_filesystem(image: &[u8]) -> Result<(), String> {
    let superblock = decode_superblock(image)?;
    if superblock.block_size != K16FS_BLOCK_SIZE as u32 {
        return Err(format!(
            "unsupported K16FS block size {}",
    ...
    if image.len() != expected_len {
        return Err(format!(
            "K16FS superblock declares {} blocks but image has {} bytes",
    ...
    if ranges_overlap(
        ...
        return Err("K16FS bitmap overlaps inode table".to_string());
    }
    let inode_capacity = inode_capacity(&superblock)?;
    if superblock.root_inode_id >= inode_capacity {
        return Err("K16FS root inode is outside inode table".to_string());
    }
    let root = decode_inode(image, &superblock, superblock.root_inode_id)?;
    if root.state != InodeState::Directory {
        return Err("K16FS root inode is not a directory".to_string());
    }
    validate_allocated_inodes(image, &superblock)?;
    Ok(())
}

fn validate_allocated_inodes(image: &[u8], superblock: &K16FsSuperblock) -> Result<(), String> {
    let inode_capacity = inode_capacity(superblock)?;
        ...
        match inode.state {
            InodeState::Free | InodeState::Deleted => {
                if inode.size_bytes != 0 || !inode.extents.is_empty() {
                    return Err(format!("K16FS inactive inode {inode_id} carries data"));
                }
            }
...
fn validate_inode_size(inode: &K16FsInode, inode_id: u32) -> Result<(), String> {
    let capacity = inode_extent_capacity(inode)?;
    if inode.size_bytes > capacity {
        return Err(format!("K16FS inode {inode_id} size exceeds extents"));
    }
    Ok(())
}
...
fn validate_directory_entries(
    ...
    for entry in entries {
        if names.iter().any(|name| name == &entry.name) {
            return Err(format!(
                "K16FS directory inode {inode_id} has duplicate entry `{}`",
                entry.name
            ));
        }
        ...
        if entry.inode_id >= inode_capacity {
            return Err(format!(
                "K16FS directory inode {inode_id} entry points outside inode table"
            ));
        }
        let target = decode_inode(image, superblock, entry.inode_id)?;
        if matches!(target.state, InodeState::Free | InodeState::Deleted) {
            return Err(format!(
                "K16FS directory inode {inode_id} entry points to inactive inode"
            ));
        }
    }
    Ok(())
...
fn encode_superblock(image: &mut [u8], superblock: &K16FsSuperblock) -> Result<(), String> {
    if image.len() < K16FS_BLOCK_SIZE {
        return Err("K16FS image is too small for superblock".to_string());
    }
    image[0..5].copy_from_slice(K16FS_MAGIC);
    image[5] = K16FS_VERSION;
    image[6] = 0;
    image[7] = 0;
...
fn encode_inode(
    ...
    if inode.extents.len() > K16FS_MAX_INLINE_EXTENTS {
        return Err(format!(
    ...
    let offset = inode_offset(superblock, inode_id)?;
    let bytes = image
        .get_mut(offset..offset + K16FS_INODE_SIZE as usize)
        .ok_or_else(|| format!("K16FS inode {inode_id} is outside filesystem"))?;
...
fn parse_parent_path(path: &str) -> Result<ParsedChildPath<'_>, String> {
    ...
    let Some(name) = components.pop() else {
        return Err("K16FS path must name an entry".to_string());
    };
    Ok(ParsedChildPath {
...
fn parse_absolute_path(path: &str) -> Result<Vec<&str>, String> {
    if !path.starts_with('/') {
        return Err("K16FS path must be absolute".to_string());
    }
    if path == "/" {
        return Ok(Vec::new());
    }
    if path.ends_with('/') {
        return Err("K16FS path must not end with `/`".to_string());
    }
    let mut components = Vec::new();
    for component in path[1..].split('/') {
        if component.is_empty() {
            return Err("K16FS path contains an empty component".to_string());
        }
        if component == "." || component == ".." {
            return Err("K16FS path component `.` or `..` is unsupported".to_string());
        }
...
fn validate_name(name: &str) -> Result<(), String> {
    if name.is_empty() {
        return Err("K16FS name must not be empty".to_string());
    }
    if name.len() > K16FS_MAX_NAME_BYTES {
        return Err(format!(
            "K16FS name `{name}` is longer than {K16FS_MAX_NAME_BYTES} bytes"
        ));
    }
    Ok(())
}
...
fn find_directory_inode(
    ...
    let inode = decode_inode(image, superblock, inode_id)?;
    if inode.state != InodeState::Directory {
        return Err("K16FS path component is not a directory".to_string());
    }
    Ok(inode_id)
}
...
fn find_directory_entry_slot(
    ...
    if directory.state != InodeState::Directory {
        return Err(format!(
            "K16FS inode {directory_inode_id} is not a directory"
        ));
    }
    if directory.size_bytes % K16FS_DIRECTORY_ENTRY_SIZE as u64 != 0 {
        return Err(format!(
            "K16FS directory inode {directory_inode_id} has unaligned size"
        ));
    }
    ...
    for extent in &directory.extents {
        for block in extent.start_block..extent.start_block + extent.block_count {
            let mut block_offset = block as usize * K16FS_BLOCK_SIZE;
            for _ in 0..K16FS_BLOCK_SIZE / K16FS_DIRECTORY_ENTRY_SIZE {
                if remaining == 0 {
                    return Err(format!("K16FS directory entry `{name}` not found"));
                }
                let bytes = image
                    .get(block_offset..block_offset + K16FS_DIRECTORY_ENTRY_SIZE)
                    .ok_or_else(|| "K16FS directory entry is truncated".to_string())?;
                ...
                block_offset += K16FS_DIRECTORY_ENTRY_SIZE;
                directory_offset += K16FS_DIRECTORY_ENTRY_SIZE;
            }
        }
    }
    Err(format!(
        "K16FS directory inode {directory_inode_id} size exceeds extents"
    ))
}
...
fn ensure_missing_entry(
    ...
    {
        return Err(format!("K16FS directory entry `{name}` already exists"));
    }
    Ok(())
}
...
fn append_directory_entry(
    ...
    directory.size_bytes = directory
        .size_bytes
        .max((slot.directory_offset + K16FS_DIRECTORY_ENTRY_SIZE) as u64);
...
fn find_free_directory_slot(image: &[u8], directory: &K16FsInode) -> Result<DirectorySlot, String> {
    ...
    for extent in &directory.extents {
        for block in extent.start_block..extent.start_block + extent.block_count {
            let block_offset = block as usize * K16FS_BLOCK_SIZE;
            for entry_offset in (0..K16FS_BLOCK_SIZE).step_by(K16FS_DIRECTORY_ENTRY_SIZE) {
                let image_offset = block_offset + entry_offset;
                let state = *image
                    .get(image_offset)
                    .ok_or_else(|| "K16FS directory extent is truncated".to_string())?;
                if state != 1 {
...
fn decode_directory_entry(bytes: &[u8]) -> Result<Option<DirectoryEntry>, String> {
    ...
    match state {
        ...
        1 => {
            ...
            if name_len == 0 || name_len > K16FS_MAX_NAME_BYTES {
                return Err("K16FS directory entry has invalid name length".to_string());
            }
            if bytes[2] != 0 || bytes[3] != 0 {
                return Err("K16FS directory entry reserved bytes must be zero".to_string());
            }
            let inode_id = read_u32(bytes, 0x04)?;
            let name_bytes = &bytes[0x08..0x08 + name_len];
            let name = std::str::from_utf8(name_bytes)
                .map_err(|_| "K16FS directory entry name is not UTF-8".to_string())?
                .to_string();
            validate_name(&name)?;
            Ok(Some(DirectoryEntry { inode_id, name }))
        }
        _ => Err(format!("unsupported K16FS directory entry state {state}")),
    }
}
...
fn encode_directory_entry(
    ...
    validate_name(name)?;
    let bytes = image
        .get_mut(offset..offset + K16FS_DIRECTORY_ENTRY_SIZE)
        .ok_or_else(|| "K16FS directory entry is outside filesystem".to_string())?;
    bytes.fill(0);
    bytes[0] = 1;
...
fn allocate_inode(image: &[u8], superblock: &K16FsSuperblock) -> Result<u32, String> {
    let inode_capacity = inode_capacity(superblock)?;
    ...
    Err("K16FS inode table is full".to_string())
}
...
fn is_block_allocated(
    ...
    if block >= superblock.total_blocks {
        return Err(format!("K16FS block {block} is outside filesystem"));
    }
    let bitmap_offset = superblock.bitmap_start_block as usize * K16FS_BLOCK_SIZE;
...
fn blocks_for_len(len: usize) -> Result<u32, String> {
    let block_count = len.div_ceil(K16FS_BLOCK_SIZE).max(1);
    u32::try_from(block_count).map_err(|_| "K16FS file is too large".to_string())
}

fn read_inode_bytes(image: &[u8], inode: &K16FsInode) -> Result<Vec<u8>, String> {
    ...
    for extent in &inode.extents {
        ...
        let bytes = image
            .get(range)
            .ok_or_else(|| "K16FS file extent is outside filesystem".to_string())?;
    ...
    Err("K16FS file size exceeds extents".to_string())
}

fn inode_extent_capacity(inode: &K16FsInode) -> Result<u64, String> {
    let mut capacity = 0_u64;
    for extent in &inode.extents {
        let bytes = u64::from(extent.block_count)
            .checked_mul(K16FS_BLOCK_SIZE as u64)
            .ok_or_else(|| "K16FS extent capacity overflows".to_string())?;
        capacity = capacity
            .checked_add(bytes)
...
fn block_range(start_block: u32, block_count: u32) -> Result<Range<usize>, String> {
    let start = start_block
        .checked_mul(K16FS_BLOCK_SIZE as u32)
        .ok_or_else(|| "K16FS block range overflows".to_string())? as usize;
    let byte_count = block_count
        .checked_mul(K16FS_BLOCK_SIZE as u32)
        .ok_or_else(|| "K16FS block range overflows".to_string())? as usize;
    let end = start
        .checked_add(byte_count)
        .ok_or_else(|| "K16FS block range overflows".to_string())?;
    Ok(start..end)
}
...
fn mark_free(image: &mut [u8], superblock: &K16FsSuperblock, block: u32) -> Result<(), String> {
    if block >= superblock.total_blocks {
        return Err(format!("K16FS block {block} is outside filesystem"));
    }
    if range_overlaps_metadata(block, 1, superblock) {
        return Err("K16FS cannot free metadata block".to_string());
    }
    let bitmap_offset = superblock.bitmap_start_block as usize * K16FS_BLOCK_SIZE;
    let byte_offset = bitmap_offset + (block as usize / 8);
    let bit = block as u8 % 8;
...
fn bitmap_blocks_for(total_blocks: u32) -> Result<u32, String> {
    let bits_per_block = (K16FS_BLOCK_SIZE * 8) as u32;
...
fn inode_table_blocks_for(inode_count: u32) -> Result<u32, String> {
    let bytes = inode_count
        .checked_mul(K16FS_INODE_SIZE)
        .ok_or_else(|| "K16FS inode table size overflows".to_string())?;
    Ok(bytes.div_ceil(K16FS_BLOCK_SIZE as u32))
}

fn inode_capacity(superblock: &K16FsSuperblock) -> Result<u32, String> {
    let bytes = superblock
        .inode_table_block_count
        .checked_mul(K16FS_BLOCK_SIZE as u32)
        .ok_or_else(|| "K16FS inode table size overflows".to_string())?;
    Ok(bytes / K16FS_INODE_SIZE)
}

fn inode_offset(superblock: &K16FsSuperblock, inode_id: u32) -> Result<usize, String> {
    let capacity = inode_capacity(superblock)?;
    if inode_id >= capacity {
        return Err(format!("K16FS inode {inode_id} is outside inode table"));
    }
    let base = superblock.inode_table_start_block as usize * K16FS_BLOCK_SIZE;
    Ok(base + inode_id as usize * K16FS_INODE_SIZE as usize)
}
...
fn validate_inode_extents(
    ...
    for extent in &inode.extents {
        if extent.block_count == 0 {
            return Err(format!("K16FS {label} has zero-sized extent"));
        }
        validate_range(
            extent.start_block,
            extent.block_count,
        ...
        if range_overlaps_metadata(extent.start_block, extent.block_count, superblock) {
            return Err(format!("K16FS {label} extent overlaps metadata"));
        }
    }
    Ok(())
}

fn range_overlaps_metadata(start: u32, count: u32, superblock: &K16FsSuperblock) -> bool {
...
fn validate_range(start: u32, count: u32, total_blocks: u32, label: &str) -> Result<(), String> {
    if count == 0 {
        return Err(format!("K16FS {label} range is empty"));
    }
    let end = start
        .checked_add(count)
        .ok_or_else(|| format!("K16FS {label} range overflows"))?;
    if end > total_blocks {
        return Err(format!("K16FS {label} outside filesystem"));
    }
    Ok(())
}
...
fn validate_metadata_range(
    ...
    if ranges_overlap(start, count, 0, 1) {
        return Err(format!("K16FS {label} overlaps superblock"));
    }
    Ok(())
}
...
fn filesystem_len(total_blocks: u32) -> Result<usize, String> {
    total_blocks
        ...
        .ok_or_else(|| "K16FS image size overflows".to_string())
}

fn read_u32(bytes: &[u8], offset: usize) -> Result<u32, String> {
    let value = bytes
        .get(offset..offset + 4)
        .ok_or_else(|| "K16FS structure is truncated".to_string())?;
