...
fn k16_link_converts_k16_object_with_abs32_relocation_to_program_k16e() {
    ...
    assert_eq!(executable.load_addr, 0x1_2000);
    assert_eq!(&bytes[0..4], b"K16E");
    assert_eq!(u32_at(&bytes, 12), 0x1_2000);
    assert_eq!(u32_at(&bytes, 24), 3);
    assert_eq!(u32_at(&bytes, 36), 0x1_2000);
    assert_eq!(
        &bytes[52..],
        &[0x01, 0xe4, 0x00, 0x20, 0x01, 0x00, 0x01, 0x00]
    );
}
...
fn k16_link_converts_k16_object_to_raw_bios_flash() {
    ...
    assert_eq!(
        bytes,
        [
            0x01, 0xef, 0x00, 0x40, 0x02, 0x00, 0x01, 0xee, 0x0e, 0x00, 0xf0, 0xff, 0x00, 0x7e,
...
fn k16_link_emits_bss_as_k16e_zero_fill_memory_tail() {
    ...
    assert_eq!(
        &bytes[52..],
        &[0x01, 0xe4, 0x08, 0x20, 0x01, 0x00, 0x01, 0x00]
    );
}
