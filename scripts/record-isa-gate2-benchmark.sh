...
SNAPSHOT="$ROOT/docs/benchmarks/isa-gate2-current.txt"
...
for tool in "$ISA_GATE2_CLANG" "$ISA_GATE2_OPT" "$ISA_GATE2_LLC" "$ISA_GATE2_LLD" \
    ...
    if ! "$tool" --version | grep -Eq '(^|[^0-9])22\.[0-9]'; then
        echo "ISA Gate 2 requires LLVM major version 22: $tool" >&2
        exit 2
    fi
done

mkdir -p "$ROOT/docs/benchmarks"
ARTIFACTS="$(mktemp -d "${TMPDIR:-/tmp}/isa-gate2-artifacts.XXXXXX")"
...
cleanup() {
    ...
    if (( completed )); then
        ...
        echo "ISA Gate 2 failed; artifacts preserved at $ARTIFACTS" >&2
    fi
    exit "$status"
}
...
dirty="clean"
if [[ -n "$(git -C "$ROOT" status --porcelain -- . ':(exclude)docs/benchmarks/isa-gate2-current.txt' 2>/dev/null || true)" ]]; then
    dirty="dirty"
fi
host="$(uname -srmo)"
rustc_version="$(rustc --version)"
cargo_version="$(cargo --version)"
clang_version="$($ISA_GATE2_CLANG --version | head -n 1)"
opt_version="$($ISA_GATE2_OPT --version | sed -n '2s/^  //p')"
llc_version="$($ISA_GATE2_LLC --version | sed -n '2s/^  //p')"
lld_version="$($ISA_GATE2_LLD --version | head -n 1)"
...
ISA_GATE2_CLANG="$ISA_GATE2_CLANG" ISA_GATE2_OPT="$ISA_GATE2_OPT" \
ISA_GATE2_LLC="$ISA_GATE2_LLC" ISA_GATE2_LLD="$ISA_GATE2_LLD" \
    "$ROOT/scripts/compile-isa-gate2-corpus.sh" "$ARTIFACTS" "$ITERATIONS"

benchmark_output="$(
    cargo run --quiet --locked --offline --release \
        --manifest-path "$ROOT/host/k16-vm/Cargo.toml" \
        --example isa_gate2_benchmarks -- "$ARTIFACTS" "$ITERATIONS" "$SAMPLES"
)"

timing_rows="$(printf '%s\n' "$benchmark_output" | awk -F '\t' '$2 == "k16-f32" || $2 == "rv32im" || $2 == "native-rust" { count += 1; if ($16 != 0 || $17 != 0) bad = 1 } END { if (bad) exit 1; print count + 0 }')"
if [[ "$timing_rows" -ne 18 ]]; then
    echo "ISA Gate 2 expected 18 allocation-free timing rows, got $timing_rows" >&2
    exit 1
fi
printf '%s\n' "$benchmark_output" | grep -q '^k16-f32[[:space:]]'
printf '%s\n' "$benchmark_output" | grep -q '^rv32im[[:space:]]'
if [[ "$(printf '%s\n' "$benchmark_output" | grep -Ec $'^decision\t(select-k16-f32|select-rv32im|inconclusive-expanded-run)$')" -ne 1 ]]; then
    echo "ISA Gate 2 benchmark did not produce exactly one valid decision" >&2
    exit 1
fi

{
    echo "ISA Gate 2 compiled-C benchmark current snapshot"
    echo
    echo "Issue: #481"
    ...
    echo "Command: K16_LLVM_BIN_DIR=\"$K16_LLVM_BIN_DIR\" bash scripts/record-isa-gate2-benchmark.sh $ITERATIONS $SAMPLES"
    echo
