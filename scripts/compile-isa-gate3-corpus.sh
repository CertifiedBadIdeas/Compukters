...
: "${ISA_GATE3_OBJCOPY:=llvm-objcopy}"
: "${ISA_GATE3_READOBJ:=llvm-readobj}"
: "${ISA_GATE3_NM:=llvm-nm}"
...
for tool in "$ISA_GATE3_CLANG" "$ISA_GATE3_OPT" "$ISA_GATE3_LLC" "$ISA_GATE3_LLD" \
    "$ISA_GATE3_OBJCOPY" "$ISA_GATE3_READOBJ" "$ISA_GATE3_NM" "$K16_LLVM_BIN_DIR/llc"; do
...
if [[ -z "${ISA_GATE3_CHECKSUM_BIN:-}" || -z "${ISA_GATE3_K16_ASSEMBLER_BIN:-}" ]]; then
    cargo build --quiet --locked --offline --manifest-path "$CRATE_MANIFEST" \
        --bin isa_gate2_checksum --bin k16_f32r32_assemble
fi
CHECKSUM_BIN="${ISA_GATE3_CHECKSUM_BIN:-$ROOT/host/k16-vm/target/debug/isa_gate2_checksum}"
K16_ASSEMBLER_BIN="${ISA_GATE3_K16_ASSEMBLER_BIN:-$ROOT/host/k16-vm/target/debug/k16_f32r32_assemble}"
for tool in "$CHECKSUM_BIN" "$K16_ASSEMBLER_BIN"; do
    if [[ ! -x "$tool" ]]; then
        echo "required ISA Gate 3 host tool is not executable: $tool" >&2
        exit 2
    fi
done
...
for workload in "${workloads[@]}"; do
    ...
    candidate_asm="$directory/k16-f32r32-lr.s"
    ...
    (
        cd "$SOURCE_ROOT"
        "$ISA_GATE3_CLANG" --target=i386-unknown-none-elf -std=c11 -O0 \
    ...
    sed -E '/^target datalayout =/d; /^target triple =/d; s/ "target-cpu"="[^"]*"//g; s/ "target-features"="[^"]*"//g' \
        "$raw_ir" > "$neutral_ir"
    "$ISA_GATE3_OPT" -passes='default<O2>' -vectorize-loops=false -vectorize-slp=false \
    ...
    "$K16_LLVM_BIN_DIR/llc" -mtriple=k16 -mattr=+f32r32lr -filetype=asm \
        "$canonical_ir" -o "$candidate_asm"
    "$K16_ASSEMBLER_BIN" --base 4096 --entry kernel --input "$candidate_asm" \
        --output "$candidate_image" --manifest "$candidate_manifest"

    "$ISA_GATE3_LLC" -mtriple=riscv32-unknown-elf -mattr=+m,-a,-c \
        -function-sections -filetype=obj "$canonical_ir" -o "$rv_object"
    "$ISA_GATE3_LLD" -m elf32lriscv -T "$SOURCE_ROOT/rv32im.ld" \
        --no-relax --fatal-warnings "$rv_object" -o "$rv_elf"
    if [[ -n "$("$ISA_GATE3_NM" --undefined-only "$rv_elf")" ]]; then
        echo "linked RV32IM artifact contains undefined symbols for $workload" >&2
        exit 1
    fi
    if "$ISA_GATE3_READOBJ" --relocations "$rv_elf" | grep -Eq '0x[0-9A-Fa-f]+ R_'; then
        echo "linked RV32IM artifact contains relocations for $workload" >&2
        exit 1
    fi

    "$ISA_GATE3_OBJCOPY" -O binary --only-section=.text "$rv_elf" "$rv_image"
    ...
    kernel_address_hex="$("$ISA_GATE3_NM" --defined-only "$rv_elf" | awk '$3 == "kernel" { print $1 }')"
...

    {
        ...
        echo "clang_version=$($ISA_GATE3_CLANG --version | head -n 1)"
        echo "opt_version=$($ISA_GATE3_OPT --version | sed -n '2s/^  //p')"
        echo "llc_version=$($ISA_GATE3_LLC --version | sed -n '2s/^  //p')"
    } > "$rv_manifest"

    {
        ...
        echo "source_sha256=$(sha256sum "$source" | awk '{print $1}')"
        echo "canonical_ir_sha256=$(sha256sum "$canonical_ir" | awk '{print $1}')"
        echo "image_sha256=$(sha256sum "$candidate_image" | awk '{print $1}')"
        echo "validation_iterations=$VALIDATION_ITERATIONS"
        echo "expected_checksum=$expected_checksum"
