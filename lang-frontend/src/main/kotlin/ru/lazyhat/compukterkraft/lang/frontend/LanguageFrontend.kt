...
private class SemanticAnalyzer(
        ...
        declarations.filterIsInstance<StructDeclaration>().forEach { declaration ->
            ...
            val symbol =
                SymbolInfo(
                    ...
                    range = declaration.range,
                    detail = "record ${declaration.name}",
                )
                ...
                is VariableDeclarationStatement -> {
                    ...
                    val symbol =
                        SymbolInfo(
                            ...
                            detail = "${if (statement.mutable) "var" else "let"} ${statement.name}: ${declaredType.displayName}",
    ...
    private fun analyzeRecordConstruction(
        ...
        if (record == null) {
            diagnostics += FrontendDiagnostic("Unknown record `${expression.typeName}`.", expression.range)
            return TypeRef("Unit")
        }
        expression.fields.forEach { field ->
            val expected = record.fields[field.name]
            if (expected == null) {
                diagnostics += FrontendDiagnostic("Record `${record.symbol.name}` has no field `${field.name}`.", field.range)
            } else {
                val actual = analyzeExpression(field.expression, scope)
                expectAssignable(actual, expected, field.range, "Record field type mismatch.")
            }
        }
        return TypeRef(record.symbol.name)
    }
...
private class Parser(
    ...
    private fun parseRecordConstruction(nameToken: Token): Expression? {
        ...
        while (!check(TokenKind.RBRACE) && !isAtEnd()) {
            val fieldName = consume(TokenKind.IDENTIFIER, "Expected record field name.") ?: return null
            consume(TokenKind.COLON, "Expected `:` after record field name.") ?: return null
            val expression = parseExpression() ?: return null
            fields += RecordFieldInitializer(fieldName.text, expression, SourceRange(fieldName.range.start, expression.range.end))
            if (!match(TokenKind.COMMA)) break
        }
        val end = consume(TokenKind.RBRACE, "Expected `}` after record construction.") ?: return null
