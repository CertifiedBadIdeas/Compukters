use rux_compiler::ruxfs::{
    ...
    RUXFS_BLOCK_SIZE, RUXFS_DEFAULT_INODE_COUNT, RUXFS_INODE_SIZE,
};

#[test]
fn ruxfs_formats_empty_extent_filesystem_with_root_directory() {
    let image = format_empty_filesystem(128).expect("filesystem formats");

    assert_eq!(image.len(), 128 * RUXFS_BLOCK_SIZE);
    assert_eq!(&image[0..5], b"RUXFS");
    let superblock = decode_superblock(&image).expect("superblock decodes");
    assert_eq!(superblock.total_blocks, 128);
    assert_eq!(superblock.block_size, RUXFS_BLOCK_SIZE as u32);
    ...
    assert_eq!(
        ...
        (RUXFS_DEFAULT_INODE_COUNT * RUXFS_INODE_SIZE).div_ceil(RUXFS_BLOCK_SIZE as u32)
    );
...
#[test]
fn ruxfs_rejects_invalid_root_inode() {
    let mut image = format_empty_filesystem(128).expect("filesystem formats");
    let superblock = decode_superblock(&image).expect("superblock decodes");
    let root_offset = (superblock.inode_table_start_block as usize * RUXFS_BLOCK_SIZE)
        + RUXFS_INODE_SIZE as usize;
