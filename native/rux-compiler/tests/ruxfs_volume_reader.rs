...
fn ruxfs_volume_reader_loads_kernel_file_from_root_partition() {
    ...
    ruxfs::write_file(&mut root, "/boot/kernel.kx", &kernel_bytes).expect("kernel writes");
    ...
    let loaded = ruxfs_volume::read_file_from_partition(&storage0, "ROOT", "/boot/kernel.kx")
...
fn ruxfs_volume_reader_reports_missing_partition_or_path_without_fallback() {
    ...
    assert!(
        ruxfs_volume::read_file_from_partition(&storage0, "DATA", "/boot/kernel.kx")
            .unwrap_err()
            .contains("RUXPT partition `DATA` not found")
    );
    assert!(
        ruxfs_volume::read_file_from_partition(&storage0, "ROOT", "/boot/kernel.kx")
            .unwrap_err()
