...
const R_RUX16_BRANCH4: u32 = 3;

#[derive(Debug, Clone)]
pub struct Rux16LinkInput<'a> {
    pub name: &'a str,
    pub bytes: &'a [u8],
}

pub fn link_rux16_objects_to_ruxe(
    inputs: &[Rux16LinkInput<'_>],
    ...
    ruxe::encode_rux16_executable(&linked.payload, abi_kind, linked.entry_pc, load_addr)
}
...
fn link_objects(objects: &[ParsedObject], load_addr: u32) -> Result<LinkedImage, String> {
    ...
    for object in objects {
        ...
        for section in &object.sections {
            ...
            let output_offset = u32::try_from(payload.len())
                .map_err(|_| "linked Rux16 payload is too large".to_string())?;
            object_offsets[section.index] = Some(output_offset);
            match section.kind {
                SHT_PROGBITS => payload.extend_from_slice(&section.bytes),
                SHT_NOBITS => {
                    let size = usize::try_from(section.size)
                        .map_err(|_| "Rux16 NOBITS section is too large".to_string())?;
                    payload.resize(
                        payload
                            .len()
                            .checked_add(size)
                            .ok_or_else(|| "linked Rux16 payload is too large".to_string())?,
                        0,
                    );
                }
    ...
    if payload.is_empty() {
        return Err("Rux16 link produced an empty payload".to_string());
    }
    if payload.len() % 2 != 0 {
        payload.push(0);
    }
    ...
    for (object_index, object) in objects.iter().enumerate() {
        for symbol in &object.symbols {
            ...
            if let Some(previous) = defined_symbols.insert(symbol.name.clone(), address) {
                return Err(format!(
                    "duplicate Rux16 symbol `{}` at {previous:#010x} and {address:#010x}",
                    symbol.name
                ));
            }
        }
    ...
    let entry_pc = *defined_symbols
        .get("_start")
        .ok_or_else(|| "Rux16 link requires defined entry symbol `_start`".to_string())?;
    ...
    match section.name.as_str() {
        ".text.rux16" => {
            if section.kind != SHT_PROGBITS
                ...
                || section.alignment != 2
            {
                return Err("unsupported .text.rux16 section attributes".to_string());
            }
            if section.size % 2 != 0 {
                return Err(".text.rux16 size must be even".to_string());
            }
        }
        ".rodata" => {
...
fn resolve_symbol(
    ...
    if symbol.section_index == SHN_UNDEF {
        return defined_symbols
            .get(&symbol.name)
            .copied()
            .ok_or_else(|| format!("{}: unresolved Rux16 symbol `{}`", object.name, symbol.name));
    }
    let section_index = usize::from(symbol.section_index);
    let Some(Some(section_offset)) = section_offsets
        .get(object_index)
...
fn apply_relocation(
    ...
    match kind {
        R_RUX16_NONE => Ok(()),
        R_RUX16_ABS32 | R_RUX16_CALL32 => {
            let value = u32::try_from(value).map_err(|_| {
                format!("{object_name}: Rux16 relocation value {value} does not fit u32")
            })?;
        ...
        R_RUX16_BRANCH4 => {
            ...
            if delta % 2 != 0 {
                return Err(format!(
                    "{object_name}: R_RUX16_BRANCH4 target {value:#010x} is not word-aligned"
                ));
            }
            let offset_words = delta / 2;
            if !(-8..=7).contains(&offset_words) {
                return Err(format!(
                    "{object_name}: R_RUX16_BRANCH4 offset {offset_words} is outside -8..=7"
                ));
            }
            let offset = usize::try_from(output_offset)
                .map_err(|_| format!("{object_name}: relocation offset is too large"))?;
        ...
        other => Err(format!("unsupported Rux16 relocation {other}")),
    }
}

fn align_payload(payload: &mut Vec<u8>, alignment: u32) -> Result<(), String> {
    if alignment == 0 {
        return Err("Rux16 alloc section alignment must be non-zero".to_string());
    }
    let alignment = usize::try_from(alignment)
        .map_err(|_| "Rux16 alloc section alignment is too large".to_string())?;
