...
pub fn create_initialized_volume(size: usize) -> Result<Vec<u8>, String> {
    if size % partition::K16PT_BLOCK_SIZE != 0 {
        return Err(format!(
            "k16vol init size must be a multiple of {} bytes",
            partition::K16PT_BLOCK_SIZE
        ));
    }
    let total_blocks = u32::try_from(size / partition::K16PT_BLOCK_SIZE)
...
pub fn put_boot(volume: &mut [u8], boot: &[u8]) -> Result<(), String> {
    ...
    let executable = k16e::decode_k16_executable(boot)?;
    if executable.abi_kind != k16e::K16eAbiKind::Bootloader {
        return Err("boot media requires K16E bootloader ABI kind".to_string());
    }
    let payload = &mut volume[payload_range];
    if payload.get(..5) != Some(partition::K16PT_MAGIC) {
        return Err("put-boot requires a K16PT partitioned volume".to_string());
    }
...
pub fn put_kernel(volume: &mut [u8], kernel: &[u8]) -> Result<(), String> {
    let payload_range = validate_volume_header(volume)?;
    let executable = k16e::decode_k16_executable(kernel)?;
    if executable.abi_kind != k16e::K16eAbiKind::Kernel {
        return Err("kernel media requires K16E kernel ABI kind".to_string());
    }
    let payload = &mut volume[payload_range];
...
pub fn inspect_boot_chain(volume: &[u8]) -> Result<String, String> {
    ...
    output.push_str(&format!(
        "BOOTLOADER K16E abi=bootloader entry_pc={:#010x} load_addr={:#010x} payload_bytes={}\n",
        bootloader.entry_pc,
    ...
    output.push_str(&format!(
        "KERNEL K16E abi=kernel entry_pc={:#010x} load_addr={:#010x} payload_bytes={}\n",
        kernel.entry_pc,
        kernel.load_addr,
        kernel.payload.len()
    ));
...
fn decode_payload_partition_table(payload: &[u8]) -> Result<partition::K16PartitionTable, String> {
    if payload.len() % partition::K16PT_BLOCK_SIZE != 0 {
        return Err("k16vol payload size is not block-aligned".to_string());
    }
    let table_bytes = payload
        .get(..partition::K16PT_BLOCK_SIZE)
        .ok_or_else(|| "k16vol payload is too small for K16PT".to_string())?;
    let table = partition::decode_partition_table(table_bytes)?;
    let total_blocks = u32::try_from(payload.len() / partition::K16PT_BLOCK_SIZE)
...
fn partition_entry_by_type(
    table: &partition::K16PartitionTable,
    ...
    table
        ...
        .ok_or_else(|| format!("K16PT {} partition not found", partition_type.tag()))
}
