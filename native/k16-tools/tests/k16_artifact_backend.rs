...
#[test]
fn k16_artifact_empty_main_halts() {
    let artifact = compile_k16_artifact("fn main() { }", K16ArtifactTarget::Boot)
        .expect("empty main compiles to K16");

    assert_eq!(artifact.target, K16ArtifactTarget::Boot);
    let executable = k16e::decode_k16_executable(&artifact.bytes).expect("K16E decodes");
    assert_eq!(executable.abi_kind, k16e::K16eAbiKind::Bootloader);
...
#[test]
fn k16_kernel_artifact_is_k16e_fixed_image() {
    let artifact = compile_k16_artifact("fn main() { }", K16ArtifactTarget::Kernel)
        .expect("empty main compiles to kernel K16E");

    assert_eq!(artifact.target, K16ArtifactTarget::Kernel);
    let executable = k16e::decode_k16_executable(&artifact.bytes).expect("K16E decodes");
...
#[test]
fn k16_program_artifact_is_k16e_program_executable() {
    let artifact = compile_k16_artifact("fn main() { }", K16ArtifactTarget::Program)
        .expect("empty main compiles to program K16E");

    assert_eq!(artifact.target, K16ArtifactTarget::Program);
    let executable = k16e::decode_k16_executable(&artifact.bytes).expect("K16E decodes");
...
#[test]
fn k16_abi_docs_define_implementation_ready_calling_convention() {
    ...
    for required in [
        ...
        assert!(
            ...
            "K16 ABI docs must contain `{required}`"
        );
    }
}

#[test]
fn k16_backend_lowers_canonical_u32_integer_ops() {
    let source = "
        fn main() {
    ...
    ";
    let artifact = compile_k16_artifact(source, K16ArtifactTarget::Bios)
        .expect("canonical u32 integer ops compile to K16");

    let disasm = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Bios)
...
#[test]
fn k16_backend_lowers_signed_and_unsigned_integer_comparisons() {
    let source = "
        fn main() {
    ...
    ";
    let artifact = compile_k16_artifact(source, K16ArtifactTarget::Bios)
        .expect("canonical integer comparisons compile to K16");
...
#[test]
fn k16_bios_firmware_source_runs_from_bios_flash() {
    let artifact = compile_bundled_k16_bios();
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 1024)
            .expect("machine boots K16 BIOS flash");

    assert_eq!(
        machine.run_boot_k16_until_signal(cpu_id).unwrap(),
        K16Signal::Halt,
    );
    assert_eq!(
...
#[test]
fn k16_bios_firmware_source_draws_no_bootable_device_screen() {
    let artifact = compile_bundled_k16_bios();
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 1024)
            .expect("machine boots K16 BIOS flash");
    ...
    assert_eq!(k16_display_row(&snapshot, 0), "K16 BIOS");
    assert_eq!(k16_display_row(&snapshot, 2), "No bootable device");
}

#[test]
fn k16_bios_firmware_source_rejects_incomplete_raw_fixed_record_storage0_media() {
    let artifact = compile_bundled_k16_bios();
...
#[test]
fn k16_bios_firmware_source_rejects_raw_fixed_record_header_fields_without_fallback() {
    let artifact = compile_bundled_k16_bios();
    let payload = compile_stage2_program();
    let invalid_headers = [
        k16_boot_media(0, 2048, 1, 1, &payload.bytes),
        k16_boot_media(2048, 0, 1, 1, &payload.bytes),
        k16_boot_media(2048, 2048, 0, 1, &payload.bytes),
        k16_boot_media(2048, 2048, 1, 0, &payload.bytes),
    ];
...
#[test]
fn k16_bios_firmware_source_rejects_raw_fixed_record_boot_payload_without_fallback() {
    let artifact = compile_bundled_k16_bios();
    let payload = compile_stage2_program();
    let payload = k16e::decode_k16_executable(&payload.bytes).expect("stage2 K16E decodes");
    let media = k16_boot_media(payload.entry_pc, payload.load_addr, 1, 1, &payload.payload);

    assert_bios_rejects_non_partitioned_boot_media(&artifact.bytes, media);
}

#[test]
fn k16_bios_firmware_source_does_not_jump_to_raw_fixed_record_boot_payload() {
    let artifact = compile_bundled_k16_bios();
    let payload = compile_stage2_program();
    let payload = k16e::decode_k16_executable(&payload.bytes).expect("stage2 K16E decodes");
    let media = k16_boot_media(payload.entry_pc, payload.load_addr, 1, 1, &payload.payload);
...
#[test]
fn k16_bios_firmware_source_uses_guest_storage0_read_helper() {
    let source = bundled_k16_bios_source();
...
fn compile_stage2_program() -> k16_tools::artifact::K16Artifact {
    compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("stage2 program compiles to K16")
}

fn assert_bios_rejects_non_partitioned_boot_media(bios_flash: &[u8], media: Vec<u8>) {
    let mut handle = K16ComputerHandle::create_k16_bios_flash_with_storage0_media(
        bios_flash,
        64 * 1024,
        1024,
        media,
    )
    .expect("machine boots K16 BIOS flash with storage0 boot media");
...
#[test]
fn k16_artifact_imports_abi_constants_from_rux_source() {
    let artifact = compile_k16_artifact(
        ...
        K16ArtifactTarget::Bios,
    )
    .expect("explicit ABI constant imports compile to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_imports_abi_namespace_constants_from_rux_source() {
    let artifact = compile_k16_artifact(
    ...
    .expect("qualified ABI namespace constants compile to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_rejects_bare_abi_constants_without_import() {
    let error = compile_k16_artifact(
        "fn main() {
            unsafe {
    ...
    assert!(
        error
            .message
            .contains("unknown K16 MMIO address `control::PANIC_CODE`"),
        "{}",
        error.message
    );
}
...
#[test]
fn k16_artifact_mmio_i32_store_runs_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi("fn main() { unsafe { mmio<i32>(debug::WRITE).store(65); } }"),
        K16ArtifactTarget::Bios,
    )
    .expect("debug MMIO store compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_mmio_u8_store_runs_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi("fn main() { unsafe { mmio<u8>(debug::WRITE).store(65u8); } }"),
        K16ArtifactTarget::Bios,
    )
    .expect("debug MMIO u8 store compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_rejects_out_of_range_u8_store_value() {
    let error = compile_k16_artifact(
...
#[test]
fn k16_artifact_loads_i32_mmio_into_local_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("i32 MMIO load into local compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_loads_u8_mmio_into_local_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("u8 MMIO load into local compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_assigns_i32_local_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("i32 local assignment compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_updates_i32_local_inside_while_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("while-local polling assignment compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_assigns_u8_local_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("u8 local assignment compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_uses_u32_local_as_mmio_address_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn write_ready() {
            unsafe {
    ...
    .expect("u32 MMIO address local compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_lowers_u32_loop_and_address_arithmetic_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn sum_table() {
            unsafe {
    ...
    .expect("u32 loop and address arithmetic compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
    machine
...
#[test]
fn k16_artifact_lowers_if_eq_condition_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("if equality condition compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_lowers_if_else_false_condition_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("if/else equality condition compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_lowers_while_eq_condition_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("while equality condition compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_lowers_logical_conditions_with_short_circuit_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn mark_x() -> i32 {
            unsafe {
    ...
    .expect("logical conditions compile to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 256)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_rejects_numeric_logical_operands_without_truthiness() {
    let error = compile_k16_artifact(
        "fn main() {
            if 1 && true {
            }
...
#[test]
fn k16_artifact_lowers_bool_locals_and_helper_returns_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn ready() -> bool {
    ...
    .expect("bool locals and helper returns compile to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 256)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_rejects_numeric_bool_initializer_without_truthiness() {
    let error = compile_k16_artifact(
        "fn main() {
            let mut ok: bool = 1;
            if ok {
...
#[test]
fn k16_artifact_const_mmio_sequence_runs_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "const O: i32 = 79;
         const K: i32 = 75;
    ...
    .expect("const-backed MMIO sequence compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_calls_unit_helper_function_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "const O: i32 = 79;
         const K: i32 = 75;
    ...
    .expect("unit helper call compiles to K16 call/ret");
    let disassembly = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Bios)
        .expect("BIOS artifact disassembles");
    assert!(
        disassembly.contains("call r"),
        "unit helper call must lower to a real K16 call:\n{disassembly}"
    );
    assert!(
        disassembly.contains("ret"),
        "unit helper body must return with K16 ret:\n{disassembly}"
    );
    assert!(
...
#[test]
fn k16_artifact_preserves_live_locals_around_unit_helper_call_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn write_bang() {
            unsafe {
    ...
    .expect("live locals survive K16 call/ret helper lowering");
    let disassembly = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Bios)
        .expect("BIOS artifact disassembles");
    assert!(
        disassembly.contains("call r"),
        "unit helper with live caller locals must lower to a real K16 call:\n{disassembly}"
    );
    assert!(
        disassembly.contains("ret"),
        "unit helper body must return with K16 ret:\n{disassembly}"
...
#[test]
fn k16_artifact_preserves_live_locals_around_argument_helper_call_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
    ...
    .expect("live locals survive K16 call ABI argument lowering");
    let disassembly = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Bios)
        .expect("BIOS artifact disassembles");
    assert!(
        disassembly.contains("call r"),
        "argument helper with live caller locals must lower to a real K16 call:\n{disassembly}"
    );
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 1024)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_preserves_helper_parameters_from_scratch_clobbering() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
...
#[test]
fn k16_artifact_lowers_early_returning_helper_control_flow_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
    ...
    .expect("early returning helper control flow compiles to K16");
    let disassembly = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Bios)
        .expect("BIOS artifact disassembles");
    assert!(
        disassembly.contains("ret"),
        "early returning helper must lower returns to K16 ret:\n{disassembly}"
    );
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 1024)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_lowers_u8_equality_helper_condition_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
    ...
    .expect("u8 equality helper condition compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 1024)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_lowers_all_returning_if_else_helper_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn choose_byte(flag: u8) -> u8 {
            if flag == 1u8 {
    ...
    .expect("all-returning if/else helper compiles to K16");
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 1024)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_rejects_returning_helper_with_missing_return_path() {
    let error = compile_k16_artifact(
        &with_computer_abi(
            "fn choose_byte(flag: u8) -> u8 {
            if flag == 1u8 {
...
#[test]
fn k16_artifact_calls_helper_parameters_and_returns_from_bios_flash() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn panic_addr() -> u32 {
    ...
    .expect("helper parameters and returns compile to call/ret K16");
    let disassembly = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Bios)
        .expect("BIOS artifact disassembles");
    assert!(
        disassembly.contains("call r"),
        "helper parameters and returns must lower through the K16 call ABI:\n{disassembly}"
    );
    assert!(
        disassembly.contains("ret"),
        "called helper bodies must return through K16 ret:\n{disassembly}"
    );
    let (mut machine, cpu_id) =
        ComputerMachine::from_k16_bios_flash(&artifact.bytes, 64 * 1024, 128)
            .expect("machine boots K16 BIOS flash");
...
#[test]
fn k16_artifact_rejects_stack_passed_helper_arguments_without_fallback() {
    let error = compile_k16_artifact(
    ...
    assert!(
        error.message.contains(
            "helper function `write_four` has 4 parameters, but the K16 call ABI supports at most 3"
        ),
        "{}",
        error.message
    );
...
#[test]
fn k16_artifact_backend_reserves_r15_for_stack_pointer() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            unsafe {
    ...
    .expect("conditional firmware compiles to K16");
    let disassembly = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Bios)
        .expect("BIOS artifact disassembles");
...
#[test]
fn k16_artifact_backend_reserves_r12_for_frame_pointer() {
    let error = compile_k16_artifact(
        &with_computer_abi(
            "fn main() {
            let mut a: u8 = 1u8;
    ...
    assert!(
        error.message.contains("K16 backend ran out of registers"),
        "{}",
        error.message
    );
}
...
#[test]
fn k16_artifact_uses_stack_backed_helper_local_from_frame_pointer() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn write_stacked() {
            let mut a: u8 = 1u8;
...
#[test]
fn k16_artifact_uses_stack_array_helper_byte_buffer_from_frame_pointer() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
            "fn write_buffered() {
...
#[test]
fn k16_artifact_lowers_address_of_stack_array_element_to_guest_address() {
    let artifact = compile_k16_artifact(
        &with_computer_abi(
...
#[test]
fn k16_artifact_reads_storage0_block_into_stack_buffer() {
    ...
    let artifact = compile_k16_artifact(&with_computer_abi(&source), K16ArtifactTarget::Bios)
    ...
    .expect("machine boots K16 BIOS flash with storage0 media");

    let signal = handle.run_k16_until_signal().unwrap();
    assert_eq!(
        signal,
        K16Signal::Halt,
...
#[test]
fn k16_artifact_rejects_recursive_helper_inline() {
    let error = compile_k16_artifact(
        "fn again() {
            again();
         }
    ...
    assert!(
        error.message.contains("recursive K16 helper call `again`"),
        "{}",
        error.message
    );
}
...
#[test]
fn k16_artifact_lowers_write_csr_builtin_from_kernel() {
    let artifact = compile_k16_artifact(
        "fn main() {
            unsafe {
                k16_write_csr(1u32, 0x4200u32);
            }
        }",
        K16ArtifactTarget::Kernel,
    )
    .expect("K16 write CSR builtin compiles");
    let disassembly = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Kernel)
        .expect("artifact disassembles");

    assert!(
        disassembly.contains("write_csr 1, r"),
        "write CSR builtin must lower to a real K16 CSR write:\n{disassembly}"
    );
}

#[test]
fn k16_artifact_lowers_read_csr_builtin_from_kernel() {
    let artifact = compile_k16_artifact(
        "fn main() {
            unsafe {
                let mut cause: u32 = k16_read_csr(2u32);
                cause = cause + 1u32;
            }
        }",
        K16ArtifactTarget::Kernel,
    )
    .expect("K16 read CSR builtin compiles");
    let disassembly = k16_disasm::disassemble_artifact(&artifact.bytes, K16ArtifactTarget::Kernel)
        .expect("artifact disassembles");

    assert!(
        disassembly.contains("read_csr r"),
        "read CSR builtin must lower to a real K16 CSR read:\n{disassembly}"
    );
}
