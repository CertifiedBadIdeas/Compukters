...
const K16FS_BLOCK_SIZE: usize = 512;
const K16FS_INODE_SIZE: usize = 64;
const K16FS_DIRECTORY_ENTRY_SIZE: usize = 64;
...
pub fn read_k16fs_file_from_partition(
    storage_media: &[u8],
    partition: &str,
    path: &str,
) -> Result<Vec<u8>, String> {
    let partition = partition_payload(storage_media, partition)?;
    read_k16fs_file(partition, path)
}
...
fn read_k16fs_file(image: &[u8], path: &str) -> Result<Vec<u8>, String> {
...
fn decode_superblock(image: &[u8]) -> Result<K16FsSuperblock, String> {
    ...
    if block_size != K16FS_BLOCK_SIZE as u32 {
        return Err(format!("unsupported K16FS block size {block_size}"));
    }
    let total_blocks = read_u32(image, 0x0c)?;
    let expected_len = block_offset(total_blocks)?;
    if image.len() != expected_len {
        return Err(format!(
            "K16FS superblock declares {total_blocks} blocks but image has {} bytes",
            image.len()
        ));
    }
...
fn find_directory_entry(
    ...
    read_directory_entries(image, superblock, directory_inode_id)?
        ...
        .ok_or_else(|| format!("K16FS directory entry `{name}` not found"))
}
...
fn decode_directory_entry(bytes: &[u8]) -> Result<Option<DirectoryEntry>, String> {
    match bytes[0] {
        ...
        state => Err(format!("unsupported K16FS directory entry state {state}")),
    }
}
...
fn decode_inode(
    ...
    let bytes = image
        .get(offset..offset + K16FS_INODE_SIZE)
...
fn inode_offset(superblock: &K16FsSuperblock, inode_id: u32) -> Result<usize, String> {
    let capacity = superblock
        ...
        .checked_mul((K16FS_BLOCK_SIZE / K16FS_INODE_SIZE) as u32)
    ...
    base.checked_add(inode_id as usize * K16FS_INODE_SIZE)
        .ok_or_else(|| "K16FS inode offset overflows".to_string())
}

fn block_offset(block: u32) -> Result<usize, String> {
    usize::try_from(block)
        .map_err(|_| "block index does not fit usize".to_string())?
        .checked_mul(K16FS_BLOCK_SIZE)
